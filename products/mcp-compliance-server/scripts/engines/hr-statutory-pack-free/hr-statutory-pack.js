'use strict';
/**
 * hr-statutory-pack.js —— 人力法定费用技能包（免费档 / 完整档共用源码）
 *
 * 谁在什么时候必须用它：**集团/多主体的 HR 与财务共享中心**，在**每年残保金申报、每月工会经费计提、年度汇算福利费限额之前**。
 * 人力口子上的"法定费用"最容易漏：**残疾人就业保障金**要按上年在职职工人数与安排残疾人比例算，
 * **工会经费**要按工资总额 2% 计提/上缴并核对返还，**职工福利费、职工教育经费**有扣除限额，
 * 超了要纳税调整 —— 每一项都有明确算式与政策口径，但**逐家主体手核既慢又容易漏**，年年重复。
 * 集团往往同时管着**好几家法人主体**，真正要做的事是"**一次跑完所有主体，每家主体一行结论**"。
 *
 * 这个包做的事**只有一件**（不是重写任何规则）：把仓库里**已有的 3 个人力法定费用检查引擎**
 * 装进一个批量壳里，`--input <目录>`（每个子目录 = 一家公司 / 一套材料）一次跑完所有公司，
 * **每家公司一行结论**。
 *   检查项：残疾人就业保障金核对、工会经费计提与上缴核对、职工福利费与教育经费限额核对。
 *   规则来源：`scripts/engine/parts/<检查项>/`（**逐字节拷贝**自那 3 个免费包，一行都没改；
 *   本包只做"目录 → 公司 → 逐项派发"的编排与汇总，外加完整档的跨公司汇总台账）。
 ...
 *     payload.text            单公司：这家公司的一套材料文本（每项用 `=== 检查项 ===` 分段）
 *     payload.objects[]       多公司：{name, files:[{name, text}]} —— 每个子目录 = 一家公司
 ...
 * ⚠️ 材料不足**绝不给结论**：某家公司没材料就单独标"未执行"，一家公司都跑不了就整体不给结论。
 * ⚠️ 本工具**不代替做账/审计/纳税申报、不出鉴证意见**，也不判断业务实质该怎么定。
 */

/* 3 个成员检查项的**中文名 / base 名 / 引擎**（引擎逐字节来自各自免费包，本包一行规则都没改） */
const MEMBERS = [
  {
    label: '残疾人就业保障金核对',
    id: 'disability-fund-check',
    entry: require('./parts/disability-fund-check/disability-fund-check.js'),
  },
  {
    label: '工会经费计提与上缴核对',
    id: 'union-reserve-check',
    entry: require('./parts/union-reserve-check/union-reserve-check.js'),
  },
  {
    label: '职工福利费与教育经费限额核对',
    id: 'welfare-limit-check',
    entry: require('./parts/welfare-limit-check/welfare-limit-check.js'),
  },
];

/* 免费档执行：3 个人力法定费用检查项，逐家公司全跑一遍（免费层的核心产出：一次跑完所有公司） */
const CHECKS_GIVEN = MEMBERS.map((m) => m.label);

/* 完整档追加（**多一种能力**，不是多几个数）：跨公司汇总台账 —— 单公司结果里根本不存在的东西 */
...
/* 如实列出**每个成员检查包自己**没做的子检查（那 3 个免费包各自的未执行项）——
...
const OUT_OF_SCOPE = [
  '判断某类支出算不算工资总额、能不能进福利费（属于**会计与税务判断**，请按公司制度与主管口径执行）',
  '代替申报残保金、工会经费或纳税申报，也不出具鉴证意见（只做表内/表间的算术与勾稽核对）',
  '核对社保/公积金缴费凭证、工会经费缴款回单的真伪，或比对社保与公积金系统的逐行明细',
  '读取社保 / 公积金 / 工会系统的导出文件（需要你先导出成文本，每家主体一个目录）',
  '判断各地残保金比例与优惠政策口径是否最新（本工具只核你给的表内数字与表间勾稽关系）',
];

/* 样例 = 3 个成员检查包各自的最小完整样例（各自带表头，用 `=== 检查项 ===` 分段）。
   这 3 份样例**都是干净稿**（生成时逐项自证过 0 条发现，见 SAMPLE_HAS_FINDINGS）。 */
const SAMPLE_TEXT = MEMBERS
  .map((m) => `=== ${m.label} ===\n${m.entry.SAMPLE_TEXT}`)
  .join('\n\n');

/* 有一处硬伤的样例：把「残疾人就业保障金核对」的合计行里 `420` 改成 `520.00`（各明细行相加 ≠ 合计）⇒ 必须报出来，并带**原文行号**（第 4 行）。 */
const DIRTY_FROM = '合计	420			6.30	3.00	402500.00	402500.00	0.00	3	3	51500000.00';
const DIRTY_TO = '合计	520.00			6.30	3.00	402500.00	402500.00	0.00	3	3	51500000.00';
const DIRTY_TEXT = SAMPLE_TEXT.replace(DIRTY_FROM, DIRTY_TO);

/* 只覆盖 3 项的样例：用来演示"材料只覆盖了一部分"时，批量壳**明确报一条**，
   而不是把"跑了 3 项"说成"3 项都核过了"。 */
...
/* 批量样例：3 家公司（干净 / 有一处硬伤 / 没交材料），用来演示"一次跑完所有公司"。 */
const SAMPLE_OBJECTS = [
  { name: '豫州制造主体（2026 年度人力费用）', files: [{ name: '人力费用材料.txt', text: SAMPLE_TEXT }] },
  { name: '中岳商贸主体（2026 年度人力费用）', files: [{ name: '人力费用材料.txt', text: DIRTY_TEXT }] },
  { name: '缺料主体（只登记未交材料）', files: [] },
];

/** 内置样例**不含发现**（3 份成员样例都是干净稿，生成时逐项自证过 0 条发现）。
...
/** 入参归一化成"公司列表"：`{text}` 是单公司，`{objects:[…]}`（或 `{orgs/clients:[…]}`）是批量。 */
  ...
  if (payload.text !== undefined || payload.content !== undefined) {
    raw.push({
      name: String(payload.object_name || payload.object || payload.org_name || payload.org
        || payload.client_name || payload.client || payload.name || '单公司').trim(),
      files: coerceFiles(payload),
    });
  }
  const out = [];
  raw.forEach((o, i) => out.push({ name: o.name || `公司${i + 1}`, files: o.files }));
  return out;
}

/** 覆盖缺口检查：公司交了材料，但只覆盖了一部分检查项 ⇒ **明确报一条**，
 *  不能因为"跑了几项"就以为 3 项都核过了。整份材料都没交的公司不在这里报（那是公司级的"未执行"）。 */
function checkClientCoverage(it) {
  ...
  return {
    ...
    line: 1,
    message: `公司「${it.client}」的材料只覆盖了 ${ran.length} / ${it.checks.length} 个人力法定费用检查项，`
...
/** 一家公司跑完全部 3 项；每一项的实际规则在 parts/ 里（本文件一行规则都没改）。
 *  派发方式：把这家公司的每一段材料依次喂给成员引擎，**第一个跑通的段**就是这一项的表；
...
function runOneObject(object) {
  ...
  if (!candidates.length || materialLines < 4) {
    return {
      ...
      reason: '没有收到这家公司的可用材料（材料为空或只有几行）',
    };
  }
  ...
  for (const m of MEMBERS) {
    ...
    if (!hit) {
      ...
      checks.push({
        ...
        missing: bestMissing || ['这家公司的材料里没有这一项需要的表头行'],
      });
      continue;
    }
  ...
  if (cover) {
    findings.push(Object.assign({}, cover, {
      ...
      source_file: '', source_line: 0, evidence: '公司级',
    }));
  }
  ...
  /* 这一行有没有被真正核对过（有材料、且至少有一项能跑）⇒ 才够格进跨公司汇总台账。
     免费档与完整档都用它：台账只收"真跑过"的公司行，没交材料的公司不进总表。 */
...
function run(payload) {
  ...
  if (!payload) {
    return insufficient([
      '一家公司的材料都没收到（objects[] 与 text 都是空）',
      '单公司用 {"text":"…"}；批量用 {"objects":[{"name":"公司名","files":[{"name":"材料.txt","text":"…"}]}]}',
    ]);
  }
  ...
  if (!objects.length) {
    return insufficient([
      '一家公司的材料都没收到（objects[] 与 text 都是空）',
      '每家公司一个子目录，目录里放这家公司的一套材料（每项用 `=== 检查项 ===` 分段，Tab 分隔最稳）',
    ]);
  }
  ...
  if (!usable.length) {
    return insufficient(
      [`全部 ${rows.length} 家公司都没有可用材料（每家公司目录里要有这家公司的材料，且每张表要有表头行）`]
        .concat(rows.map((r) => `公司「${r.object}」：没有可用材料`)),
      '把每家公司的材料放进各自子目录后再跑；材料不足时本工具不做任何认定、也不给任何结论。');
  }
  ...
  const result = {
    ...
    note: `本次对 ${rows.length} 家公司逐个跑了 ${CHECKS_GIVEN.length} 个人力法定费用检查项（每个成员包内部还有 `
      + `${TOTAL_SUB_CHECKS / CHECKS_GIVEN.length} 项子检查）；未执行的检查项见 scope.checks_not_run 与 `
      + 'scope.sub_checks_not_run。',
    disclaimer: '只把各公司材料里的表内/表间算术与勾稽核一遍，结论都带**原文文件与行号**、可由第三方复算；'
      + '**不代替审计、不出具鉴证意见**，也不判断收费/定价/医保政策口径。',
  };

  /* 分层：
     · 免费档（免费包里就是下面这两行 + 再下面那句 return，付费块被整块摘掉）：
       只报**实际执行**的那 3 项，未执行项如实列出（说明文本，不是实现）。
     · 完整档（本文件）：在上面那层之上再挂一层跨公司汇总台账
       （公司 × 检查项总表、人力法定费用风险排序清单、跨公司共性问题归类、Markdown / CSV 导出）。 */
  ...
  /* 完整档：在上面那层之上再挂一层跨公司汇总台账
     （公司 × 检查项总表、人力法定费用风险排序清单、跨公司共性问题归类、Markdown / CSV 导出）。 */
  const ledgerAttached = ledger_hook(result, findings, rows, payload) === true;

  /* 分档后的"实际执行 / 未执行"：只认**钩子到底挂上台账没有**（免费档返回 false ⇒ 保持 3 项）。
