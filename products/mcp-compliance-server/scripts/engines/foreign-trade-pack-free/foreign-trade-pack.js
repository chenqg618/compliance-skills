'use strict';
/**
 * foreign-trade-pack-full.js —— 外贸与出口技能包（免费档 / 完整档共用源码）
 *
 * 谁在什么时候必须用它：**外贸公司 / 出口企业的外贸会计、单证员、报关与退税岗，
 * 在每期报关出口、收汇核销、出口退税申报、进口清关、国际运费与保险结算这些固定节点上**。
 * 要核的从来不是一张表，而是一整套互相关联的外贸出口底稿：出口退税的免抵退勾稽、
 * 报关单与出口发票的单证一致性、报关与收汇核销、外币结算与汇兑损益、进口关税与消费税、
 * 货运险投保与货损理赔、运单回单与代收货款、运费对账、物流仓储费、进项税认证抵扣、
 * 发票作废红冲、发票领用存、寄售代销结算、履约保函台账 ——
 * 每张表都有自己的勾稽关系，**逐张手核既慢又容易漏**，而且每个报关批次、每一期都要重复一遍；
 * 少算一笔免抵退税额、多抵一笔进项税、单证字段对不上，后果都比多花半小时严重得多。
 *
 * 这个包做的事**只有一件**（不是重写任何规则）：把仓库里**已有的 14 个外贸与出口检查引擎**
 * 装进一个批量壳里，`--input <目录>`（每个子目录 = 一个核对对象 / 一套材料）一次跑完所有对象，
 * **每个核对对象一行结论**。
 *   规则来源：`scripts/engine/parts/<检查项>/`（**逐字节拷贝**自那 14 个免费包，一行都没改；
 *   本包只做"目录 → 核对对象 → 逐项派发"的编排与汇总）。
 * * ⚠️ 本文件是 **免费档子集**：只实现免费检查项；**完整档（付费）的实现不在这个包里**。
 * `CHECKS_WITHHELD` 只是"未执行的检查项"的**说明文本**，不是实现。

 * 契约（与仓库其它引擎一致，别改名字）：
 *   run(payload) -> {status:'success', result} | {status:'insufficient_input', missing, advice}
 *     payload.text            单对象：该对象的一套外贸出口材料文本（每项用 `=== 检查项 ===` 分段）
 *     payload.objects[]       多对象：{name, files:[{name, text}]} —— 每个子目录 = 一个核对对象
 ...
 * ⚠️ 本工具**不代替报关、退税申报、外汇核销或审计**，也不判断某笔业务该不该退税、单证是真是假。
 */

const MEMBERS = [
  {
    label: '出口退税核算核对',
    id: 'export-rebate-check',
    entry: require('./parts/export-rebate-check/export-rebate-check.js'),
  },
  {
    label: '出口退税申报单证一致性核对',
  ...
  {
    label: '进口税费核算核对',
    id: 'import-duty-check',
    entry: require('./parts/import-duty-check/import-duty-check.js'),
  },
  {
    label: '货运险投保与货损理赔核对',
    id: 'cargo-insurance-claim-check',
    entry: require('./parts/cargo-insurance-claim-check/cargo-insurance-claim-check.js'),
  },
  {
    label: '运单回单与代收货款核对',
    id: 'waybill-pod-cod-check',
    entry: require('./parts/waybill-pod-cod-check/waybill-pod-cod-check.js'),
  },
  {
    label: '运费对账核对',
...
/* 免费档执行：14 个外贸与出口检查项，逐个核对对象全跑一遍（免费层的核心产出：一次跑完所有对象） */
...
const OUT_OF_SCOPE = [
  '判断报关单、出口发票、提单、保单、原产地证等单证本身的真伪，或比对海关 / 税务 / 银行 / 船公司的逐行明细',
  '判断某笔出口业务该适用哪一版退税率、商品编码归类是否正确、是否满足退税条件（属于海关与税务口径判断）',
  '代替报关、出口退税申报、外汇核销或审计程序，或出具鉴证意见与审计意见',
  '判断信用证条款是否可接受、有没有软条款（属于银行与商务条款判断，请按信用证与合同约定执行）',
  '读取报关系统 / 退税申报系统 / ERP / 银行网银的导出文件（需要你先导出成文本，每个核对对象一个目录）',
  '判断外汇汇率走势、现在该不该锁汇（本工具只核你给的表内数字与表间勾稽关系）',
];
...
/* 有一处硬伤的样例：把「出口退税核算核对」里 A001 那笔的**不得免征和抵扣税额**从
   213000.00 改成 210000.00（出口离岸价 1000000.00 × 汇率 7.10 × (征税率 13% − 退税率 10%)
   = 213000.00，与 210000.00 不符）⇒ 必须报出来，并带**原文行号**。 */
const DIRTY_FROM = 'A001\t1000000.00\t7.10\t13%\t10%\t213000.00\t710000.00\t500000.00\t500000.00\t210000.00';
const DIRTY_TO = 'A001\t1000000.00\t7.10\t13%\t10%\t210000.00\t710000.00\t500000.00\t500000.00\t210000.00';
const DIRTY_TEXT = SAMPLE_TEXT.replace(DIRTY_FROM, DIRTY_TO);

/* 只覆盖 3 项的样例：用来演示"材料只覆盖了一部分"时，批量壳**明确报一条**，
   而不是把"跑了 3 项"说成"14 项都核过了"。 */
const PARTIAL_TITLES = ['出口退税核算核对', '出口报关与收汇核销核对', '运费对账核对'];
const PARTIAL_TEXT = SAMPLE_TEXT.split(/(?=^=== )/m)
  .filter((s) => PARTIAL_TITLES.some((t) => s.trim().indexOf(`=== ${t} ===`) === 0))
  .join('\n');

/* 批量样例：3 个核对对象（干净 / 有一处硬伤 / 没交材料），用来演示"一次跑完所有对象"。 */
const SAMPLE_OBJECTS = [
  { name: '华越进出口（2026-03 出口批次）', files: [{ name: '外贸出口材料.txt', text: SAMPLE_TEXT }] },
  { name: '远洋商贸（2026-03 出口批次）', files: [{ name: '外贸出口材料.txt', text: DIRTY_TEXT }] },
  { name: '缺料批次（只登记未交材料）', files: [] },
];
...
/** 入参归一化成"核对对象列表"：`{text}` 是单对象，`{objects:[…]}`（或 `{clients:[…]}`）是批量。 */
function normalizeObjects(payload) {
  ...
  const out = [];
  raw.forEach((o, i) => out.push({ name: o.name || `核对对象${i + 1}`, files: o.files }));
  return out;
}
...
function checkClientCoverage(it) {
  ...
  return {
    ...
    line: 1,
    message: `核对对象「${it.client}」的材料只覆盖了 ${ran.length} / ${it.checks.length} 个外贸与出口检查项，`
...
/** 一个核对对象跑完全部 14 项；每一项的实际规则在 parts/ 里（本文件一行规则都没改）。
...
function runOneObject(object) {
  ...
  if (!candidates.length || materialLines < 5) {
    return {
      ...
      reason: '没有收到这个核对对象的可用材料（材料为空或只有几行）',
    };
  }
  ...
  for (const m of MEMBERS) {
    ...
    if (!hit) {
      ...
      checks.push({
        ...
        missing: bestMissing || ['这个核对对象的材料里没有这一项需要的表头行'],
      });
      continue;
    }
...
function run(payload) {
  ...
  if (!payload) {
    return insufficient([
      '一个核对对象的材料都没收到（objects[] 与 text 都是空）',
      '单对象用 {"text":"…"}；批量用 {"objects":[{"name":"核对对象名","files":[{"name":"材料.txt","text":"…"}]}]}',
    ]);
  }
  ...
  if (!objects.length) {
    return insufficient([
      '一个核对对象的材料都没收到（objects[] 与 text 都是空）',
      '每个核对对象一个子目录，目录里放该对象的一套外贸出口材料（每项用 `=== 检查项 ===` 分段，Tab 分隔最稳）',
    ]);
  }
  ...
  if (!usable.length) {
    return insufficient(
      [`全部 ${rows.length} 个核对对象都没有可用材料（每个对象目录里要有该对象的外贸出口材料，且每张表要有表头行）`]
        .concat(rows.map((r) => `核对对象「${r.object}」：没有可用材料`)),
      '把每个核对对象的出口材料放进各自子目录后再跑；材料不足时本工具不做任何认定、也不给任何结论。');
  }
  ...
  const result = {
    service_type: 'FOREIGN_TRADE_PACK',
    scope: {
    ...
    note: `本次对 ${rows.length} 个核对对象逐个跑了 ${CHECKS_GIVEN.length} 个外贸与出口检查项（每个成员包内部还有 `
    ...
    disclaimer: '只把各对象材料里的表内/表间算术与勾稽核一遍，结论都带**原文文件与行号**、可由第三方复算；'
      + '**不代替报关、退税申报、外汇核销与审计**，也不判断某笔业务该不该退税、单证是真是假。',
  };
  ...
  /* 分层：
     ...
     · 完整档（本文件）：在上面那层之上再挂一层跨对象汇总台账
       （对象 × 检查项总表、出口风险排序清单、跨对象共性问题归类、Markdown / CSV 导出）。 */
