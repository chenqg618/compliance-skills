...
<!-- MARKET_COUNT_START -->
或直接到 <https://www.skillhub.cn/> 搜同一批名字（**公开 API 回读**：已上架 **152 个免费技能** + **31 个完整档**）。
<!-- MARKET_COUNT_END -->
...
| `accrued-expense-amortization-check-free` | 预提待摊明细表逐项核对，每条结论都带原文依据，不需要付款，也不需要注册 |
...
| `agency-monthly-close-free` | 一次跑完所有代账客户的月度自查（7 个行业月度自查包 × 目录里全部客户），每个客户一行结论，每条结论都带原文文件与行号，不需要付款，也不需要注册 |
...
| `audit-adjustment-pack-free` | 把一套审计与账务调整底稿按 14 项检查逐个核一遍，每个被审计单位一行结论，结论都带原文文件与行号，不需要付款、不需要注册 |
| `autoshop-parts-labor-check-free` | 维修工单配件与工时费明细表逐项核对（配件小计复算、工时费复算、工单总额勾稽、重复配件行、负数行与关键字段空缺），每条结论都带原文行号，不需要付款，也不需要注册 |
...
| `compliance-expiry-check-free` | 证照与特种设备年检到期台账逐项核对，有效期至 = 上次检验日期 + 检验周期、距到期天数 = 有效期至 − 基准日、状态与到期情况是否一致，并检出日期倒挂、检验周期非正、重复证… |
| `consignment-settlement-check-free` | 寄售代销结算台账逐行复算（结存、应结金额、手续费、合计勾稽），每条结论都带原文行号，不需要付款，也不需要注册 |
| `construction-material-check-free` | 材料需用计划与实际消耗对照表逐项核对（损耗量复算、损耗率与上限比对、实耗超计划、合计勾稽、重复与空缺），每条结论都带原文行号，不需要付款，也不需要注册 |
...
| `cost-allocation-check-free` | 成本分配表逐项核对，每条结论都带原文依据，不需要付款，也不需要注册 |
| `cost-inventory-pack-free` | 一次跑完一套成本存货底稿的 14 项自查（库存账实 / 出入库与加权平均成本 / 存货跌价准备 / 报废与审批 / BOM 用量与损耗 / 材料领用与定额 / 材料成本差异 /… |
| `courier-settlement-check-free` | 网点运费结算单与代收货款明细表逐项核对，复算运费、代收货款扣减与应结金额，查计费重量少计、重复运单与空缺，每条结论都带原文行号与运单号，不需要付款，也不需要注册 |
| `cross-border-ad-fee-check-free` | 平台结算单与广告费账单逐项核对，每条结论都带原文依据，不需要付款，也不需要注册 |
| `cutoff-test-check-free` | 截止性测试底稿逐项核对，每条结论都带原文依据，不需要付款，也不需要注册 |
...
| `doc-compliance-pack-free` | 把一堆别人交上来的发票与单据按 14 项单据合规检查逐份材料核一遍，每份材料一行结论，结论都带原文文件与行号，不需要付款、不需要注册 |
...
| `ecom-platform-pack-free` | 把一套电商与平台结算材料按 14 项检查逐个客户核一遍，每个客户一行结论，结论都带原文文件与行号，不需要付款、不需要注册 |
| `ecom-return-refund-check-free` | 退货退款与库存回冲明细表逐项核对，每条结论都带原文行号，不需要付款，也不需要注册 |
...
| `equity-method-investment-check-free` | 权益法核算表逐项核对，每条结论都带原文依据，不需要付款，也不需要注册 |
...
| `export-tax-refund-check-free` | 出口退税申报与单证对照表逐项核对（退税额复算、合计勾稽、单号一一对应、档位一致性、单证缺失），每条结论都带原文行号，不需要付款，也不需要注册 |
| `factory-overhead-check-free` | 制造费用归集与分摊表逐项核对（分摊率与分摊金额复算、分摊合计勾稽、吸收率与吸收差异、重复与空缺），每条结论都带原文行号，不需要付款，也不需要注册 |
| `fee-budget-pack-free` | 把一套费用与预算材料按 14 项检查逐个客户核一遍，每个客户一行结论，结论都带原文文件与行号，不需要付款、不需要注册 |
...
| `foreign-trade-pack-free` | 把一套外贸出口材料按 14 项检查逐个核一遍，每个核对对象一行结论，结论都带原文文件与行号，不需要付款、不需要注册 |
| `franchise-deposit-check-free` | 加盟商保证金与费项结算明细表逐行复算（保证金余额勾稽、费项金额=基数×费率、合计行勾稽、重复计费、扣罚超限、缺格与负数），每条结论都带原文行号与加盟商，不需要付款，也不需要注册 |
| `franchise-royalty-check-free` | 加盟结算表逐项核对，每条结论都带原文行号，不需要付款，也不需要注册 |
| `freight-loss-claim-check-free` | 货运破损理赔与承运商扣款明细表逐项核对，复算理赔金额、查多扣与重复运单、赔付比例超上限、关键字段缺失或负数，每条结论都带原文行号与运单号，不需要付款，也不需要注册 |
...
| `goodwill-impairment-check-free` | 商誉减值测试底稿逐项核对，每条结论都带原文依据，不需要付款，也不需要注册 |
...
| `insurance-commission-check-free` | 保险佣金与手续费结算明细表逐项核对，每条结论都带原文行号，不需要付款，也不需要注册 |
...
| `inventory-count-variance-check-free` | 存货盘点表逐项核对，每条结论都带原文依据，不需要付款，也不需要注册 |
...
| `ip-annuity-check-free` | 知识产权年费与续展台账逐项核对，算下次缴费日、已缴合计与到期天数，查重复申请号、空缺、周期与日期倒挂，每条结论都带原文行号，不需要付款，也不需要注册 |
| `labor-cost-allocation-check-free` | 工时与人工成本分摊表逐项核对，每条结论都带原文依据，不需要付款，也不需要注册 |
| `land-appreciation-check-free` | 土地增值税预缴与清算表逐项核对（扣除项目合计、增值额、增值率、档位应纳税额、应预缴税额、已预缴差异、合计行、重复与空缺），每条结论都带原文行号，不需要付款，也不需要注册 |
| `late-fee-check-free` | 滞纳金计算表逐项核对，每条结论都带原文依据，不需要付款，也不需要注册 |
| `lease-deferred-pack-free` | 把一份合同的一套租赁与递延材料按 2 项跨期核对逐合同核一遍，每份合同一行结论，结论都带原文文件与行号，不需要付款、不需要注册 |
...
| `logistics-fuel-card-check-free` | 油卡充值与油耗明细表逐项核对，复算卡余额勾稽、百公里油耗与合计行，查重复加油、油耗超标、扣款不一致与里程回退，每条结论都带原文行号与车牌号，不需要付款，也不需要注册 |
...
| `low-value-consumables-check-free` | 低值易耗品台账逐项核对，每条结论都带原文依据，不需要付款，也不需要注册 |
...
| `manufacturing-ops-pack-free` | 把一整套制造与生产底稿按 14 项检查逐个对象核一遍，每个对象一行结论，结论都带原文文件与行号，不需要付款、不需要注册 |
...
| `membership-liability-check-free` | 会员卡销售与核销明细表逐项核对，核销复算、负债滚存勾稽、重复与作废口径，每条结论都带原文行号，不需要付款，也不需要注册 |
...
| `paid-in-capital-check-free` | 股东出资明细表逐项核对，每条结论都带原文依据，不需要付款，也不需要注册 |
...
| `payroll-hr-pack-free` | 把一套薪酬社保材料按 14 项检查逐个客户核一遍，每个客户一行结论，结论都带原文文件与行号，不需要付款、不需要注册 |
...
| `procurement-payable-pack-free` | 把一套采购与付款底稿按 14 项检查逐个核一遍，每个对象一行结论，结论都带原文文件与行号，不需要付款、不需要注册 |
| `production-yield-scrap-check-free` | 生产投入产出与报废表逐项核对，每条结论都带原文依据，不需要付款，也不需要注册 |
| `progress-output-check-free` | 进度款申请与产值确认表逐项核对（申请 vs 确认、确认值×比例、累计滚动、合计勾稽），每条结论都带原文行号，不需要付款，也不需要注册 |
...
| `project-settlement-pack-free` | 把一套工程与项目结算材料按 14 项检查逐个核一遍，每个项目一行结论，结论都带原文文件与行号，不需要付款、不需要注册 |
...
| `recon-receivables-pack-free` | 把一套往来材料按 14 项检查逐个客户核一遍，每个客户一行结论，结论都带原文文件与行号，不需要付款、不需要注册 |
| `recruitment-agency-fee-check-free` | 猎头服务费账单与保证期明细表逐项核对，每条结论都带原文行号与候选人，不需要付款、不需要注册，材料不出本机 |
| `related-party-fund-occupation-check-free` | 关联方往来台账逐项核对，每条结论都带原文依据，不需要付款，也不需要注册 |
| `related-party-transaction-check-free` | 关联交易台账逐项核对，每条结论都带原文依据，不需要付款，也不需要注册 |
| `rent-collection-check-free` | 租金收缴与欠租台账核对表逐项核对，每条结论都带原文依据，不需要付款，也不需要注册 |
| `rent-deposit-refund-check-free` | 押金保证金台账与退还明细表逐项核对（应退金额复算、扣除金额带依据与上限、退还超期、重复退还、台账余额勾稽、字段缺失与负数），每条结论都带原文行号，不需要付款，也不需要注册 |
...
| `revenue-recognition-pack-free` | 把一个项目的一套收入材料按 7 项收入确认与结算核对逐项目核一遍，每个项目一行结论，结论都带原文文件与行号，不需要付款、不需要注册 |
...
| `saas-usage-billing-check-free` | 订阅用量与超额计费明细表逐项核对，逐行复算超额用量与超额费用、核对账单金额勾稽，查套餐买大了、重复订阅与空缺，每条结论都带原文行号与订阅号，不需要付款，也不需要注册 |
| `saas-vendor-invoice-check-free` | 把软件服务商发来的账单与用量明细表逐行核一遍 —— 账单金额复算、计费数量与用量或席位对照、合计行勾稽、重复计费、超合同单价、关键字段空缺，每条结论都带原文行号 |
...
| `sales-rebate-check-free` | 返利台账逐项核对，每条结论都带原文依据，不需要付款，也不需要注册 |
| `scrap-sale-check-free` | 废料边角料台账逐行复算与勾稽核对，每条结论都带原文行号，不需要付款，也不需要注册 |
| `share-based-payment-check-free` | 股份支付明细表逐项核对，每条结论都带原文依据，不需要付款，也不需要注册 |
| `shared-cost-pack-free` | 把一家门店的一套费用材料按 6 项公共费用核对逐门店核一遍，每家门店一行结论，结论都带原文文件与行号，不需要付款、不需要注册 |
...
| `store-gross-margin-check-free` | 门店销售与成本毛利表逐项核对，每条结论都带原文行号，不需要付款，也不需要注册 |
...
| `subsidy-acceptance-check-free` | 政府补助台账与验收资料对照表逐行复算与勾稽，列出到账差额、支出缺口、验收资料缺件与重复申报，每条结论都带原文行号 |
...
| `tax-filing-pack-free` | 一次跑完一套涉税申报材料的 14 项自查（进项抵扣 / 进项转出 / 附加税费 / 印花税 / 加计扣除 / 出口退税单证 / 企业所得税 …），每个对象一行结论，每条结论都带… |
...
| `tax-risk-benefit-pack-free` | 把一家客户的一套税费材料按 5 项税费自查逐客户核一遍，每家客户一行结论，结论都带原文文件与行号，不需要付款、不需要注册 |
| `tax-risk-indicator-check-free` | 税务风险指标自查表逐项核对，每条结论都带原文依据，不需要付款，也不需要注册 |
| `teacher-hour-pay-check-free` | 课时消耗与教师课时费明细表逐项核对（课时费复算、正课/试听/补课口径、双线课耗勾稽、合计复核、保底单价、重复与空缺），每条结论都带原文行号，不需要付款，也不需要注册 |
...
| `ticket-revenue-check-free` | 票务收入与渠道结算明细表的逐行复算与勾稽，每条结论都带原文行号，不需要付款，也不需要注册 |
...
| `travel-toll-pack-free` | 把一个部门的一套差旅材料按 2 项差旅与通行核对逐部门核一遍，每个部门一行结论，结论都带原文文件与行号，不需要付款、不需要注册 |
| `treasury-bill-pack-free` | 把一套资金与票据底稿按 14 项检查逐个核一遍，每个对象一行结论，结论都带原文文件与行号，不需要付款、不需要注册 |
...
| `vat-deduction-addon-check-free` | 加计抵减台账逐项核对，每条结论都带原文依据，不需要付款，也不需要注册 |
