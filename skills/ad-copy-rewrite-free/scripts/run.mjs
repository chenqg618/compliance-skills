...
const USAGE = `广告文案违规体检（免费） —— 本机执行的AI核对
  ...
  **不需要付款**：不需要注册、不需要 API Key，也不联网；文案不出本机。
...
function main() {
  ...
  if (loaded && loaded.note) console.log(`（${loaded.note}）`);
  console.log('');
  // 本次**不含任何购买引导**：完整档（服务端订阅）的官方购买指令还没登记，
  // 就宁可先不给出口，也绝不写一个不存在的商品名／商品号／价格（编错等于把买家送错门）。
  console.log('公开入口与官方购买指令：https://chenqg618.github.io/compliance-skills/buy.html（也可看本包 SKILL.md 的「需要完整档时（可选）」一节）。');  console.log(JSON.stringify(view.result, null, 2));
  ...
  console.log('公开入口与官方购买指令：https://chenqg618.github.io/compliance-skills/buy.html（也可看本包 SKILL.md 的「需要完整档时（可选）」一节）。');  return 0;
}
