 ...
 * run.mjs —— 广告文案违规体检（免费）
 *
 * 全部检查都在**本机**完成：调用同目录下的 engine/ad-copy-rewrite.js（纯 Node 标准库实现）。
 ...
 *   node scripts/run.mjs --input my-copy.json
 *   node scripts/run.mjs --input my-copy.json --json
 *
 * 退出码：
 *   0  已执行检查（结果里有问题项或没有问题项都算执行成功）
 *   1  没给入参
 *   3  材料不足（空 / 只有空白 / 只有一个字符 / 类型不对）—— 此时不给"未发现问题"的结论
 *   4  入参文件读不到或内容无法解析
 *   9  未预期错误
 */
...
const ENGINE = require(path.join(HERE, 'engine', 'ad-copy-rewrite.js'));

const CAPABILITY = '广告文案违规体检（免费）';

const NOTE = '本版本只执行上面列出的检查项，核对全部在本机完成（不发送你的材料；结束后有一次匿名次数统计）；'
  + '未执行的检查项已如实列出，不会用默认值编造结论。';

const SAMPLE = { text: ENGINE.SAMPLE_TEXT };   // 样例是一段**合规的**广告文案（引擎导出，跑出来 0 命中）

const USAGE = `广告文案违规体检（免费） —— 本机执行的AI核对

  **不需要付款**：不需要注册、不需要 API Key，核对过程在本机完成；文案不出本机。
...
入参写法：
  · {"text":"秋季新款针织开衫，100% 羊毛，7 天无理由退换。"}
  · 多行文案直接换行写进 text（一行一句最清晰，报出来的行号就对着你自己的原文）

参数：
  -i, --input    入参 JSON 文件路径（也接受直接粘贴的广告文案纯文本）
      --sample   使用内置样例
...
/** 读入参：合法 JSON 就按其结构走；不是 JSON 就当作纯文本文案（例如直接粘贴的一段广告语） */
function loadInput(file) {
  ...
  try {
    ...
    return { payload: { text: stripped }, note: '入参文件不是合法 JSON，已按纯文本广告文案处理' };
  }
}

/** 材料不足：说清楚缺什么，并且明确不给结论 */
function reportInsufficient(args, outcome, loaded) {
  const missing = (outcome && outcome.missing) || ['入参无法解析成可判定的广告文案'];
  const advice = (outcome && outcome.advice)
    || '请把广告文案贴进来：可用 {"text": "…"}，或先用 --sample 看看需要什么格式。';
  ...
  const view = {
    ...
    omitted_findings: 0,
    note: '材料不足，本次没有执行任何检查，因此不出结论：'
      + '既不做"违规"的认定，也不做"合规"的认定，更不会输出"未发现问题"。',
    result: {
  ...
  console.log('（检查项依赖你提供的广告文案：材料不足时不做任何认定，也不套用默认值。）');
...
function main() {
  ...
  if (args.input) {
    ...
    console.error('缺少入参。用 --input 指定广告文案文件，或用 --sample 自检。');
    console.error('');
    console.error(USAGE);
    return 1;
  }
  ...
  try {
    ...
    const mark = pathx.join(os.tmpdir(), `skillpay-choice-ad-copy-rewrite-free${sid ? '-' + sid : ''}.mark`);
    ...
    if (off) {
      ...
      console.log('请选择：A 继续免费（本次结果不受影响）　B 订阅完整版 ￥29/月（按月订阅，随时可停）');
  ...
  // __UPSELL__
  console.log('如需完整档：广告文案违规体检 · 订阅版（SkillPay 商品号 S0806000206864842）—— 本版本未执行的 5 项检查在完整档里都有。');
