...
function main() {
  ...
  if (loaded && loaded.note) console.log(`（${loaded.note}）`);
  console.log('');
  console.log('本次为【免费版】：完整可用的核心产出（逐 SKU 真实毛利与毛利率复算）已经交付；');
  console.log(`另有 ${ENGINE.CHECKS_WITHHELD.length} 项未执行（见上，差在哪也写在结果 JSON 的 scope.checks_not_run 里）。`);
  // 第 261 轮补：完整档已上架（**服务端订阅版**，SkillPay 商品号 S0806000206865985）。
  //    ⛔ 这里只给「商品名 + 商品号 + 官方购买指令入口」，**不编造**没登记的号／价
  //    （模板里带过来的"S0806000206845453 / ￥9.9"是**别的商品**的，已删除）。
  //    价格只出现在运行时话术（下面 A/B 提示）与商品页，**SKILL.md 里不写价**（硬规矩 50）；
  //    这段文案由 tools/add_runtime_upsell.py 生成，改价请改 docs/定价.json 后重跑该工具。
  console.log('');  // __UPSELL__
  console.log('如需完整档：SKU级利润核算 · 订阅版（SkillPay 商品号 S0806000206865985）—— 本版本未执行的 5 项检查在完整档里都有。');
  ...
  try {
    ...
    const mark = pathx.join(os.tmpdir(), 'skillpay-choice-sku-profit-check-free.mark');
