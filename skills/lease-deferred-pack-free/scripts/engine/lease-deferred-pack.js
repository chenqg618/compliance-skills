'use strict';
/**
 * lease-deferred-pack.js —— 租赁与递延收益技能包（免费档 / 完整档共用源码）
 *
 * 谁在什么时候必须用它：**有租赁与政府补助的总账会计**，在**每月摊销与年度汇算之前**。
 * 跨期项目最容易"摊错"：**租赁负债与使用权资产**要按期初余额、利率、期限算利息与折旧，
 * **政府补助递延收益**要按与资产/收益相关的口径分期结转，还要核"结转额 + 余额 = 补助总额"，
 * 每一项都有明确算式，但**逐份合同手核既慢又容易漏**，而且月月重复。
 * 一家公司往往同时有**好几份租赁合同与补助文件**，真正要做的是"**一次跑完所有合同，每份合同一行结论**"。
 *
 * 这个包做的事**只有一件**（不是重写任何规则）：把仓库里**已有的 2 个租赁与递延检查引擎**
 * 装进一个批量壳里，`--input <目录>`（每个子目录 = 一家合同 / 一套材料）一次跑完所有合同，
 * **每家合同一行结论**。
 *   检查项：租赁负债与使用权资产核对、政府补助递延收益核对。
 *   规则来源：`scripts/engine/parts/<检查项>/`（**逐字节拷贝**自那 2 个免费包，一行都没改；
 *   本包只做"目录 → 合同 → 逐项派发"的编排与汇总，外加完整档的跨合同汇总台账）。
 ...
 *     payload.text            单合同：这家合同的一套材料文本（每项用 `=== 检查项 ===` 分段）
 *     payload.objects[]       多合同：{name, files:[{name, text}]} —— 每个子目录 = 一家合同
 ...
 * ⚠️ 材料不足**绝不给结论**：某家合同没材料就单独标"未执行"，一家合同都跑不了就整体不给结论。
...
const MEMBERS = [
  {
    label: '租赁负债与使用权资产核对',
    id: 'lease-liability-check',
    entry: require('./parts/lease-liability-check/lease-liability-check.js'),
  },
  {
    label: '政府补助递延收益核对',
    id: 'gov-subsidy-deferred-income-check',
    entry: require('./parts/gov-subsidy-deferred-income-check/gov-subsidy-deferred-income-check.js'),
  },
];

/* 免费档执行：2 个租赁与递延检查项，逐家合同全跑一遍（免费层的核心产出：一次跑完所有合同） */
const CHECKS_GIVEN = MEMBERS.map((m) => m.label);

/* 完整档追加（**多一种能力**，不是多几个数）：跨合同汇总台账 —— 单合同结果里根本不存在的东西 */
...
const OUT_OF_SCOPE = [
  '判断租赁该不该上表、补助与资产相关还是与收益相关（属于**会计判断**，请按准则与公司政策执行）',
  '代替审计或出具鉴证意见（只做表内/表间的算术与勾稽核对）',
  '核对租赁合同、补助批文、银行回单的真实性，或比对合同系统与资金系统的逐行明细',
  '读取合同系统 / 资金系统的导出文件（需要你先导出成文本，每份合同一个目录）',
  '判断补助是否需要纳税、按什么口径纳税（属于税务判断，请咨询税务师）',
];
...
/* 有一处硬伤的样例：把「租赁负债与使用权资产核对」的合计行里 `1387.31` 改成 `1487.31`（各明细行相加 ≠ 合计）⇒ 必须报出来，并带**原文行号**（第 5 行）。 */
const DIRTY_FROM = '合计				1387.31	24000.00			9600.00			';
const DIRTY_TO = '合计				1487.31	24000.00			9600.00			';
...
/* 批量样例：3 家合同（干净 / 有一处硬伤 / 没交材料），用来演示"一次跑完所有合同"。 */
const SAMPLE_OBJECTS = [
  { name: '豫州总部租赁合同（2026-04 摊销）', files: [{ name: '租赁与递延材料.txt', text: SAMPLE_TEXT }] },
  { name: '中岳产业园补助文件（2026-04 结转）', files: [{ name: '租赁与递延材料.txt', text: DIRTY_TEXT }] },
  { name: '缺料合同（只登记未交材料）', files: [] },
];
...
/** 入参归一化成"合同列表"：`{text}` 是单合同，`{objects:[…]}`（或 `{orgs/clients:[…]}`）是批量。 */
  ...
  if (payload.text !== undefined || payload.content !== undefined) {
    raw.push({
      name: String(payload.object_name || payload.object || payload.org_name || payload.org
        || payload.client_name || payload.client || payload.name || '单合同').trim(),
      files: coerceFiles(payload),
    });
  }
  const out = [];
  raw.forEach((o, i) => out.push({ name: o.name || `合同${i + 1}`, files: o.files }));
  return out;
}

/** 覆盖缺口检查：合同交了材料，但只覆盖了一部分检查项 ⇒ **明确报一条**，
 *  不能因为"跑了几项"就以为 2 项都核过了。整份材料都没交的合同不在这里报（那是合同级的"未执行"）。 */
function checkClientCoverage(it) {
  ...
  return {
    ...
    line: 1,
    message: `合同「${it.client}」的材料只覆盖了 ${ran.length} / ${it.checks.length} 个租赁与递延检查项，`
...
/** 一家合同跑完全部 2 项；每一项的实际规则在 parts/ 里（本文件一行规则都没改）。
 *  派发方式：把这家合同的每一段材料依次喂给成员引擎，**第一个跑通的段**就是这一项的表；
...
function runOneObject(object) {
  ...
  if (!candidates.length || materialLines < 4) {
    return {
      ...
      reason: '没有收到这家合同的可用材料（材料为空或只有几行）',
    };
  }
  ...
  for (const m of MEMBERS) {
    ...
    if (!hit) {
      ...
      checks.push({
        ...
        missing: bestMissing || ['这家合同的材料里没有这一项需要的表头行'],
      });
      continue;
    }
  ...
  if (cover) {
    findings.push(Object.assign({}, cover, {
      ...
      source_file: '', source_line: 0, evidence: '合同级',
    }));
  }
  ...
  /* 这一行有没有被真正核对过（有材料、且至少有一项能跑）⇒ 才够格进跨合同汇总台账。
     免费档与完整档都用它：台账只收"真跑过"的合同行，没交材料的合同不进总表。 */
...
function run(payload) {
  ...
  if (!payload) {
    return insufficient([
      '一家合同的材料都没收到（objects[] 与 text 都是空）',
      '单合同用 {"text":"…"}；批量用 {"objects":[{"name":"合同名","files":[{"name":"材料.txt","text":"…"}]}]}',
    ]);
  }
  ...
  if (!objects.length) {
    return insufficient([
      '一家合同的材料都没收到（objects[] 与 text 都是空）',
      '每家合同一个子目录，目录里放这家合同的一套材料（每项用 `=== 检查项 ===` 分段，Tab 分隔最稳）',
    ]);
  }
  ...
  if (!usable.length) {
    return insufficient(
      [`全部 ${rows.length} 家合同都没有可用材料（每家合同目录里要有这家合同的材料，且每张表要有表头行）`]
        .concat(rows.map((r) => `合同「${r.object}」：没有可用材料`)),
      '把每家合同的材料放进各自子目录后再跑；材料不足时本工具不做任何认定、也不给任何结论。');
  }
  ...
  const result = {
    ...
    note: `本次对 ${rows.length} 家合同逐个跑了 ${CHECKS_GIVEN.length} 个租赁与递延检查项（每个成员包内部还有 `
      + `${TOTAL_SUB_CHECKS / CHECKS_GIVEN.length} 项子检查）；未执行的检查项见 scope.checks_not_run 与 `
      + 'scope.sub_checks_not_run。',
    disclaimer: '只把各合同材料里的表内/表间算术与勾稽核一遍，结论都带**原文文件与行号**、可由第三方复算；'
  ...
  /* 分层：
     ...
     · 完整档（本文件）：在上面那层之上再挂一层跨合同汇总台账
       （合同 × 检查项总表、租赁与递延风险排序清单、跨合同共性问题归类、Markdown / CSV 导出）。 */
