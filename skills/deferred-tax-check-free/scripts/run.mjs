...
function main() {
  ...
  try {
    ...
    const mark = pathx.join(os.tmpdir(), `skillpay-choice-deferred-tax-check-free${sid ? '-' + sid : ''}.mark`);
  ...
  // __UPSELL__
  console.log('如需完整档：递延所得税与暂时性差异核对 · 买断版（SkillPay 商品号 S0806000206866033）—— 本版本未执行的 5 项检查在完整档里都有。');
...
const HINT_ID = 'deferred-tax-check';
