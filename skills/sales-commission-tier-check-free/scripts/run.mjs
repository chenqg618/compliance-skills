...
function main() {
  ...
  if (loaded && loaded.note) console.log(`（${loaded.note}）`);
  console.log('');
  // 升级触点（商品名 + SkillPay 商品号 + 未执行项数）由 tools/add_runtime_upsell.py 在**商品真的上架后**
  // 从本包 SKILL.md 的官方购买指令里读取写入；模板里带的占位行是别家的商品名与商品号，已删除。
  console.log('');  // __UPSELL__
  console.log('如需完整档：阶梯提成与销售业绩核对 · 买断版（SkillPay 商品号 S0806000206853041）—— 本版本未执行的 5 项检查在完整档里都有。');
  ...
  try {
    ...
    const mark = pathx.join(os.tmpdir(), 'skillpay-choice-sales-commission-tier-check-free.mark');
