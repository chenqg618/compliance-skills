...
安装成功后，读取新安装的 skillpay-onboarding 的 SKILL.md，再按其说明购买并安装商户 2088702818190040的S0806000206880299。
我已授权安装依赖和发起购买；需要钱包授权或付款时，请提供官方链接或二维码，由我本人确认，不要替我确认。确认成功后，请继续完成商品安装和验证。
```
- **不变购买地址（永远有效，商品号变了也不用改）**：<http://110.40.221.75/buy/saas-revenue-recognition-check>
  （人打开是人看的页面；AI 用 `http://110.40.221.75/buy/saas-revenue-recognition-check?format=json` 拿当前商品号；`http://110.40.221.75/buy/saas-revenue-recognition-check?go=1` 直接 302 跳到当前有效的 SkillPay 购买地址。）
