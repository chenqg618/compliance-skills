...
function main() {
  ...
  if (loaded && loaded.note) console.log(`（${loaded.note}）`);
  console.log('');
  // __VERSION_CHOICE__ —— 版本选择协议：结果之后提示一次 A/B，同一会话最多 1 次，绝不阻断结果
  try {
    const os = require('node:os');
    const fsx = require('node:fs');
    const pathx = require('node:path');
    const mark = pathx.join(os.tmpdir(), 'skillpay-choice-owner-supplied-material-check-free.mark');
    if (!fsx.existsSync(mark)) {
      try { fsx.writeFileSync(mark, String(Date.now())); } catch (e) { /* 写不了标记不影响结果 */ }
      console.log(`本次为【免费版】：已执行全部 ${view.checks_given.length} 项免费检查，`
        + `另有 ${view.checks_withheld.length} 项未执行（见上）。`);
      console.log('请选择：A 继续免费（本次结果不受影响）　B 买断完整版（一次付费、长期使用）');
      console.log('（同一会话最多提示一次；选 A 或不做选择都不会影响本次结果）');
    }
  } catch (e) { /* 提示失败绝不影响结果交付 */ }
  console.log('完整档是另一个独立商品，商品名「甲供材料与分包领用核对 · 买断版」；'
    + '公开入口与官方购买指令见 https://chenqg618.github.io/compliance-skills/buy.html');
  console.log('');
  console.log('');  // __UPSELL__
  console.log('如需完整档：甲供材料与分包领用核对 · 买断版（SkillPay 商品号 S0806000206861580）—— 本版本未执行的 6 项检查在完整档里都有。');
