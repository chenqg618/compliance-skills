...
/**
 * run.mjs —— 甲供材料与分包领用核对（免费版）
 *
 * 全部检查都在**本机**完成：调用同目录下的 engine/owner-supplied-material-check.js（纯 Node 标准库实现）。
 ...
 *   node scripts/run.mjs --input my-material.json
 *   node scripts/run.mjs --input my-material.json --json
 *
 * 退出码：
 *   0  已执行检查（有问题项或没有问题项都算执行成功）
 *   1  没给入参
 *   3  材料不足（空 / 只有空白 / 认不出表头 / 没有明细行）—— 此时不给"未发现问题"的结论
 *   4  入参文件读不到或内容无法解析
 *   9  未预期错误
 */
...
const ENGINE = require(path.join(HERE, 'engine', 'owner-supplied-material-check.js'));

const CAPABILITY = '甲供材料与分包领用核对（免费版）';
...
const USAGE = `甲供材料与分包领用核对（免费版） —— 本机执行的AI核对

  不需要付款：不需要注册、不需要 API Key，材料不出本机（运行结束后有一次匿名次数统计，可用 --no-stats 关闭）。
  本版本执行的检查项：${ENGINE.CHECKS_GIVEN.join('、')}
  本版本未执行的检查项：${ENGINE.CHECKS_WITHHELD.join('、')}
...
入参写法：
  · {"text":"（把你那张甲供材台账的表头和若干行一起复制进来，Tab 分隔最稳）"}
  · 直接从 Excel 把甲供材台账（含表头）复制成文本粘进来即可
...
/** 读入参：合法 JSON 就按其结构走；不是 JSON 就当作纯文本台账材料 */
...
function reportInsufficient(args, outcome, loaded) {
  const missing = (outcome && outcome.missing) || ['入参无法解析成可核对的甲供材台账'];
  const advice = (outcome && outcome.advice)
    || '请把甲供材台账（含表头）贴进来：可用 {"text": "…"}，或先用 --sample 看看需要什么格式。';
  ...
  console.log('（检查项依赖你提供的甲供材台账：进场 / 退库 / 领用数量、单价与金额、结算抵扣金额；'
    + '材料不足或认不出表头时不做任何认定，也不套用默认值。）');
...
function main() {
  ...
  if (args.input) {
    ...
    console.error('缺少入参。用 --input 指定甲供材台账文件，或用 --sample 自检。');
    console.error('');
    console.error(USAGE);
    return 1;
  }
  ...
  if (view.checks_withheld.length) {
    console.log(`本次未执行的检查项：${view.checks_withheld.join('、')}`);
  }
  ...
  try {
    ...
    const mark = pathx.join(os.tmpdir(), `skillpay-choice-owner-supplied-material-check-free${sid ? '-' + sid : ''}.mark`);
    ...
    if (off) {
      ...
      console.log('');
      console.log('本次为【免费版】：上面就是免费版的完整结果（已执行全部免费检查项）；另有 6 项完整版会补上（差在哪见上）。');
  ...
  // __UPSELL__
  console.log('如需完整档：甲供材料与分包领用核对 · 买断版（SkillPay 商品号 S0806000206881851）—— 本版本未执行的 6 项检查在完整档里都有。');
...
const HINT_ID = 'owner-supplied-material-check';
