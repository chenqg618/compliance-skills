...
/**
 * run.mjs —— 电商与平台结算技能包（免费版）
 *
 * 全部检查都在**本机**完成：调用同目录下的 engine/ecom-platform-pack.js（纯 Node 标准库实现）。
 * 那个引擎内部装的是仓库里**已有的 14 个电商与平台结算检查引擎**（逐字节拷贝，规则一行没改），
 ...
 *   node scripts/run.mjs --input 客户目录           # 批量：每个子目录 = 一个客户/一套材料
 *   node scripts/run.mjs --input 单客户材料.json    # 单客户：JSON 或纯文本
 *   node scripts/run.mjs --input 客户目录 --json    # 机器可读
 *
 * 客户目录怎么放（每个子目录里放该客户的一套电商与平台结算材料，每项用 `=== 检查项 ===` 分段，Tab 分隔最稳）：
 *   客户目录/
...
const ENGINE = require(path.join(HERE, 'engine', 'ecom-platform-pack.js'));

const CAPABILITY = '电商与平台结算技能包（免费版）';

const NOTE = '本次按客户逐个跑全部 14 个电商与平台结算检查项，全部在本机完成（核对过程不外发材料、不写文件）；'
...
const USAGE = `电商与平台结算技能包（免费版） —— 本机执行的AI核对
...
入参写法：
  · 批量（推荐）：--input 指向一个目录，里面每个子目录就是一个客户，子目录里放该客户的一套电商与平台结算材料
  · 单客户：{"text":"（把该客户的表按 === 检查项 === 分段贴进来，Tab 分隔最稳）"}
...
function reportResult(view) {
  ...
  console.log(`本次执行的检查项（${view.checks_given.length} 个电商与平台结算检查项）：${view.checks_given.join('、')}`);
...
function main() {
  ...
  try {
    ...
    const mark = pathx.join(os.tmpdir(), `skillpay-choice-ecom-platform-pack-free${sid ? '-' + sid : ''}.mark`);
  ...
  // __UPSELL__
  console.log('如需完整档：电商与平台结算技能包 · 买断版（SkillPay 商品号 S0806000206878600）—— 本版本未执行的 4 项检查在完整档里都有。');
...
const HINT_ID = 'ecom-platform-pack';
