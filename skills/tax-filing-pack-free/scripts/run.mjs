...
/**
 * run.mjs —— 税务与涉税申报技能包（免费版）
 *
 * 全部检查都在**本机**完成：调用同目录下的 engine/tax-filing-pack.js（纯 Node 标准库实现）。
 * 那个引擎内部装的是仓库里**已有的 14 个涉税检查引擎**（逐字节拷贝，规则一行没改），
 * 本包只做"对象目录 → 逐对象跑 14 项涉税检查 → 每个对象一行结论"这件编排工作。
 *
 * 刻意不做的事：
 ...
 *   node scripts/run.mjs --input 对象目录           # 批量：每个子目录 = 一个客户/一套材料
 *   node scripts/run.mjs --input 单对象材料.json    # 单对象：JSON 或纯文本
 *   node scripts/run.mjs --input 对象目录 --json    # 机器可读
 *
 * 对象目录怎么放（每个子目录里放该对象的一套申报材料，每张表用 `=== 检查名 ===` 分段，Tab 分隔最稳）：
 *   对象目录/
 *     豫州商贸有限公司/申报材料.txt
 *     中岳建材有限公司/申报材料.txt
...
const ENGINE = require(path.join(HERE, 'engine', 'tax-filing-pack.js'));

const CAPABILITY = '税务与涉税申报技能包（免费版）';

const NOTE = '本次按对象逐个跑全部 14 个涉税检查项，全部在本机完成（核对过程不外发材料、不写文件）；'
...
const USAGE = `税务与涉税申报技能包（免费版） —— 本机执行的AI核对
...
入参写法：
  · 批量（推荐）：--input 指向一个目录，里面每个子目录就是一个对象，子目录里放该对象的一套申报材料
  · 单对象：{"text":"（把该对象的表按 === 检查名 === 分段贴进来，Tab 分隔最稳）"}
...
function reportResult(view) {
  ...
  console.log(`本次执行的检查项（${view.checks_given.length} 个涉税检查项）：${view.checks_given.join('、')}`);
...
function main() {
  ...
  try {
    ...
    const mark = pathx.join(os.tmpdir(), `skillpay-choice-tax-filing-pack-free${sid ? '-' + sid : ''}.mark`);
  ...
  // __UPSELL__
  console.log('如需完整档：税务与涉税申报技能包 · 买断版（SkillPay 商品号 S0806000206878593）—— 本版本未执行的 4 项检查在完整档里都有。');
