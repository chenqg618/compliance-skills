 ...
 * run.mjs —— 广告文案合规预检（免费）
 *
 * 全部检查都在**本机**完成：调用同目录下的 engine/ad-compliance.js（纯 Node 标准库实现）。
 ...
 *   1  没给入参
 *   3  材料不足（空 / 只有空白 / 只有一个字符 / 类型不对）—— 此时不给"合规"的结论
 *   4  入参文件读不到
 *   9  未预期错误
 */
...
const ENGINE = require(path.join(HERE, 'engine', 'ad-compliance.js'));

const CAPABILITY = '广告文案合规预检（免费）';
...
const USAGE = `广告文案合规预检（免费） —— 本机执行的AI核对
  ...
  **完全免费**：不需要注册、不需要 API Key，核对过程在本机完成；文案不出本机。
...
参数：
  -i, --input    入参 JSON 文件路径（也接受直接粘贴文案的纯文本文件）
      --sample   使用内置样例
...
function loadInput(file) {
  ...
  try {
    ...
    return { payload: { text: stripped }, note: '入参文件不是合法 JSON，已按纯文本文案处理' };
  }
}

/** 材料不足：说清楚缺什么，并且明确不给结论 */
function reportInsufficient(args, outcome, loaded) {
  const missing = (outcome && outcome.missing) || ['入参无法解析成可判定的文案'];
  const advice = (outcome && outcome.advice)
    || '请把待检文案（标题、正文、口播稿、标签可拼接）作为 text 传入，或先用 --sample 看看需要什么格式。';
  ...
  if (args.json) {
    console.log(JSON.stringify({
      ...
      missing,
      advice,
      note: '材料不足，本次没有执行任何检查，因此不出结论：既不做"合规"的认定，也不做"违规"的认定。',
    }, null, 2));
    return 3;
  }

  console.log('材料不足，本次没有执行任何检查，因此不出结论。');
  console.log('（检查项依赖你提供的文案：材料不足时不做任何认定，也不套用默认值。）');
...
function main() {
  ...
  try {
    ...
    const mark = pathx.join(os.tmpdir(), `skillpay-choice-adcheckup-content-compliance-free${sid ? '-' + sid : ''}.mark`);
...
const HINT_ID = 'adcheckup-content-compliance';
