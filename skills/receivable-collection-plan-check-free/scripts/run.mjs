...
function main() {
  ...
  if (loaded && loaded.note) console.log(`（${loaded.note}）`);
  console.log('');
  // 升级触点由 `tools/add_runtime_upsell.py` 在商品**真的上架后**写入（本包尚未上架，
  // 脚手架从模板抄来的那行带着别的商品的商品号与不存在的商品名，已删）。
  console.log('');  // __UPSELL__
  console.log('如需完整档：应收账款催收计划与回款核对 · 买断版（SkillPay 商品号 S0806000206851267）—— 本版本未执行的 5 项检查在完整档里都有。');
  ...
  try {
    ...
    const mark = pathx.join(os.tmpdir(), 'skillpay-choice-receivable-collection-plan-check-free.mark');
