 ...
 * run.mjs —— 报销单合规预检（免费）
 *
 * 全部检查都在**本机**完成：调用同目录下的 engine/expense-compliance.js（纯 Node 标准库实现）。
...
const ENGINE = require(path.join(HERE, 'engine', 'expense-compliance.js'));

const CAPABILITY = '报销单合规预检（免费）';
...
const USAGE = `报销单合规预检（免费） —— 本机执行的AI核对
...
function reportInsufficient(args, outcome, loaded) {
  ...
  const advice = (outcome && outcome.advice)
    || '请提供至少两张单据（一张报销单 + 至少一张发票；一个文本里用空行分隔，或用 documents 数组），或先用 --sample 看看需要什么格式。';
...
function main() {
  ...
  try {
    ...
    const mark = pathx.join(os.tmpdir(), `skillpay-choice-expense-compliance-free${sid ? '-' + sid : ''}.mark`);
  ...
  // __UPSELL__
  console.log('如需完整档：报销单合规预检 · 买断版（SkillPay 商品号 S0806000206880348）—— 本版本未执行的 8 项检查在完整档里都有。');
...
const HINT_ID = 'expense-compliance';
