...
/**
 * run.mjs —— 应收账款坏账准备计提核对（免费）
 *
 * 全部检查都在**本机**完成：调用同目录下的 engine/bad-debt-provision-check.js（纯 Node 标准库实现）。
...
const ENGINE = require(path.join(HERE, 'engine', 'bad-debt-provision-check.js'));

const CAPABILITY = '应收账款坏账准备计提核对（免费）';

const NOTE = '本版本只执行上面列出的检查项，核对全部在本机完成（不发送你的材料；结束后有一次匿名次数统计）；'
  + '未执行的检查项已如实列出，不会用默认值编造结论。';

const SAMPLE = { text: ENGINE.SAMPLE_TEXT };   // 样例本身就是一张带表头的坏账准备计提表

const USAGE = `应收账款坏账准备计提核对（免费） —— 本机执行的AI核对
...
入参写法：
  ...
  · 直接把坏账准备计提表（**含表头**）从 Excel 复制成文本粘进来即可，Tab 分隔最稳

参数：
  -i, --input    入参 JSON 文件路径（也接受直接粘贴的坏账准备计提表纯文本，需含表头）
      --sample   使用内置样例
...
function reportInsufficient(args, outcome, loaded) {
  ...
  const advice = (outcome && outcome.advice)
    || '请把坏账准备计提表（含表头）贴进来：可用 {"text": "…"}，或先用 --sample 看看需要什么格式。';
  ...
  console.log('（检查项依赖你提供的坏账准备计提表：材料不足或认不出表头时不做任何认定，也不套用默认值。）');
...
function main() {
  ...
  if (args.input) {
    ...
    console.error('缺少入参。用 --input 指定坏账准备计提表文件，或用 --sample 自检。');
    console.error('');
    console.error(USAGE);
    return 1;
  }
  ...
  try {
    ...
    const mark = pathx.join(os.tmpdir(), `skillpay-choice-bad-debt-provision-check-free${sid ? '-' + sid : ''}.mark`);
  ...
  // __UPSELL__
  console.log('如需完整档：应收账款坏账准备计提核对 · 买断版（SkillPay 商品号 S0806000206877437）—— 本版本未执行的 5 项检查在完整档里都有。');
...
const HINT_ID = 'bad-debt-provision-check';
