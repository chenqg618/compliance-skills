...
const ENGINE = require(path.join(HERE, 'engine', 'contract-comparison.js'));

const CAPABILITY = '多份合同条款差异比对（免费）';
...
入参两种写法：
  · {"contracts":[{"name":"合同A","text":"第三条 服务期限…"},{"name":"合同B","text":"…"}]}
  · 或直接给纯文本：每份合同用单独一行「合同一」「合同二」这样的分节标题开头（或空行分隔）

参数：
  -i, --input    入参 JSON 文件路径（也接受直接粘贴的纯文本；
                 每份合同用单独一行的「合同一」这样的分节标题开头）
      --sample   使用内置样例
...
function reportInsufficient(args, outcome, loaded) {
  ...
  const advice = (outcome && outcome.advice)
    || '请提供至少两份合同（一个文本里用「合同一」「合同二」分节，或用 contracts 数组），或先用 --sample 看看需要什么格式。';
...
function main() {
  ...
  try {
    ...
    const mark = pathx.join(os.tmpdir(), `skillpay-choice-contract-comparison-free${sid ? '-' + sid : ''}.mark`);
  ...
  // __UPSELL__
  console.log('如需完整档：多份合同条款差异比对 · 买断版（SkillPay 商品号 S0806000206880345）—— 本版本未执行的 4 项检查在完整档里都有。');
...
const HINT_ID = 'contract-comparison';
