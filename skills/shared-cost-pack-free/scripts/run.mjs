...
const NOTE = '本次按门店逐个跑全部 6 个公共费用检查项，全部在本机完成（不联网、不外发材料、不写文件）；'
...
function main() {
  ...
  try {
    ...
    if (off) {
      ...
      console.log('');
      console.log('本次为【免费版】：上面就是免费版的完整结果（6 个公共费用检查项 × 全部门店；未执行项已如实列出）。');
      console.log('买断版另加一层能力：跨门店汇总台账（门店 × 检查项总表、公共费用风险排序清单、共性问题归类、逐门店处理项、Markdown/CSV 导出）。');
