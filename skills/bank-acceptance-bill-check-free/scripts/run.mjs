...
/**
 * run.mjs —— 银行承兑汇票台账与到期兑付核对（免费）
 *
 * 全部检查都在**本机**完成：调用同目录下的 engine/bank-acceptance-bill-check.js（纯 Node 标准库实现）。
...
const ENGINE = require(path.join(HERE, 'engine', 'bank-acceptance-bill-check.js'));

const CAPABILITY = '银行承兑汇票台账与到期兑付核对（免费）';

const NOTE = '本版本只执行上面列出的检查项，核对全部在本机完成（不发送你的材料；结束后有一次匿名次数统计）；'
  + '未执行的检查项已如实列出，不会用默认值编造结论。';

const SAMPLE = { text: ENGINE.SAMPLE_TEXT };   // 样例取自引擎：一张带表头的银行承兑汇票台账（票号、出票日、到期日、期限月数、票据金额、出票人、
// 背书转让链、状态、保证金比例、已缴保证金、账面应收票据余额、备注）

const USAGE = `银行承兑汇票台账与到期兑付核对（免费） —— 本机执行的AI核对
...
入参写法：
  ...
  · 直接把台账（**含表头**）从 Excel 复制成文本粘进来即可，Tab 分隔最稳
...
function reportInsufficient(args, outcome, loaded) {
  ...
  const advice = (outcome && outcome.advice)
    || '请把票据台账（含表头：票号、出票日、到期日、期限月数、票据金额）贴进来：可用 {"text": "…"}，或先用 --sample 看看需要什么格式。';
  ...
  console.log('（检查项依赖你提供的票据台账：材料不足或认不出表头时不做任何认定，也不套用默认值。）');
...
function main() {
  ...
  if (args.input) {
    ...
    console.error('缺少入参。用 --input 指定票据台账文件，或用 --sample 自检。');
    console.error('');
    console.error(USAGE);
    return 1;
  }
  ...
  if (view.checks_withheld.length) {
    ...
    console.log(`（本次未执行的检查项共 ${view.checks_withheld.length} 项，已如实列出；本次结果不受影响。）`);
  }
  console.log('执行方式：核对在本机用 Node 标准库完成，不发送你的材料、没有次数上限；结束后向服务端做一次匿名次数统计（前 3 次无任何返回）');
  if (loaded && loaded.note) console.log(`（${loaded.note}）`);
  console.log('');
  console.log('如需买断版：商品名「银行承兑汇票台账与到期兑付核对 · 买断版」。');
  ...
  try {
    ...
    const mark = pathx.join(os.tmpdir(), `skillpay-choice-bank-acceptance-bill-check-free${sid ? '-' + sid : ''}.mark`);
  ...
  // __UPSELL__
  console.log('如需完整档：银行承兑汇票台账与到期兑付核对 · 买断版（SkillPay 商品号 S0806000206881836）—— 本版本未执行的 6 项检查在完整档里都有。');
...
const HINT_ID = 'bank-acceptance-bill-check';
