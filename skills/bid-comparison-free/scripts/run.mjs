 ...
 * run.mjs —— 多家报价横向比价（免费）
 *
 * 全部检查都在**本机**完成：调用同目录下的 engine/bid-comparison.js（纯 Node 标准库实现）。
...
const ENGINE = require(path.join(HERE, 'engine', 'bid-comparison.js'));

const CAPABILITY = '多家报价横向比价（免费）';

const NOTE = '本版本只执行上面列出的检查项，核对全部在本机完成（不发送你的材料；结束后有一次匿名次数统计）；'
  + '未执行的检查项已如实列出，不会用默认值编造结论。';

const SAMPLE = { bidders: ENGINE.SAMPLE_BIDDERS };   // 样例里两张单据用空行分隔

const USAGE = `多家报价横向比价（免费） —— 本机执行的AI核对
...
入参两种写法：
  · {"documents":[{"type":"商业发票","text":"…"},{"type":"装箱单","text":"…"}]}
  · 或直接给纯文本，**单据之间空一行**

参数：
  -i, --input    入参 JSON 文件路径（也接受直接粘贴单据全文的纯文本文件；
                 多张单据之间用**空行**分隔）
      --sample   使用内置样例
...
function reportInsufficient(args, outcome, loaded) {
  ...
  const advice = (outcome && outcome.advice)
    || '请提供至少两家报价：bidders 数组（每项含 name 与 items），或用 text、各家之间空一行，或先用 --sample 看看需要什么格式。';
  ...
  console.log('（检查项依赖你提供的单据正文：材料不足时不做任何认定，也不套用默认值。）');
...
function main() {
  ...
  if (args.input) {
    ...
    console.error('缺少入参。用 --input 指定单据文件，或用 --sample 自检。');
    console.error('');
    console.error(USAGE);
    return 1;
  }
  ...
  try {
    ...
    const mark = pathx.join(os.tmpdir(), `skillpay-choice-bid-comparison-free${sid ? '-' + sid : ''}.mark`);
  ...
  // __UPSELL__
  console.log('如需完整档：多家报价横向比价 · 买断版（SkillPay 商品号 S0806000206878565）—— 本版本未执行的 5 项检查在完整档里都有。');
