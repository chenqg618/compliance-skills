 ...
 *   ① 应结补贴 = 实际销量 × 单件补贴
 *   ② 封顶：应结补贴不得高于活动封顶金额 —— ⚠️ **本免费版未执行封顶判断**
 *      （只写进 `CHECKS_WITHHELD`，不实现、不出结论；免费版实际只核 ① 与 ③）
 ...
 * `purchase-rebate-check` 核**采购返利**；本能力核的是**促销补贴的应结与核销**（按销量计提）。
 * ⚠️ 「应结补贴是否超过活动封顶」本免费版**不核**（见 `CHECKS_WITHHELD`：封顶校验）。
...
function run(payload) {
  ...
  const result = {
    ...
    summary: {
      ...
      basis: '应结补贴 = 实际销量 × 单件补贴（**本免费版未执行封顶判断**：应结补贴是否超过「活动封顶」不核）；'
        + '差异 = 应结补贴 − 已结补贴；合计行逐列复核（单件补贴与封顶列不参与求和）。',
    },
  ...
  if (findings.length === 0) {
    result.note = '本次实际执行的全部检查项都通过了（**本免费版未执行封顶判断**：应结补贴是否超过「活动封顶」不在本次结果里，'
      + '见 checks_withheld / checks_not_run）。这只说明**这张表按上面写明的口径算得对**，'
