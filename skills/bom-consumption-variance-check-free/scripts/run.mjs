...
/**
 * run.mjs —— BOM用量与损耗差异核对（免费版）
 *
 * 全部检查都在**本机**完成：调用同目录下的 engine/bom-consumption-variance-check.js（纯 Node 标准库实现）。
 ...
 *   node scripts/run.mjs --input my-workorder.json
 *   node scripts/run.mjs --input my-workorder.json --json
 *
 * 退出码：
 *   0  已执行检查（结果里有问题项或没有问题项都算执行成功）
 *   1  没给入参
 *   3  材料不足（空 / 只有空白 / 只有一行没有表头 / 类型不对 / 没有可核对要素）—— 此时不给"未发现问题"的结论
 *   4  入参文件读不到或内容无法解析
 *   9  未预期错误
 */
...
const ENGINE = require(path.join(HERE, 'engine', 'bom-consumption-variance-check.js'));

const CAPABILITY = 'BOM用量与损耗差异核对（免费版）';

const NOTE = '本版本只执行上面列出的检查项，核对全部在本机完成（不发送你的材料；结束后有一次匿名次数统计）；'
  + '未执行的检查项已如实列出，不会用默认值编造结论。';

// 样例本身就是一张带表头的工单领料明细（标准用量 = 单位用量 × 产出数量、实际领料与退料、损耗率与超耗、
// 替代料与串料、按工单/产品归集用量差异），并且**按本工具的口径算得对**（干净样例）。
const SAMPLE = { text: ENGINE.SAMPLE_TEXT };

const MAT = '生产工单领料明细（工单号、产品名称、物料编码、BOM归属工单、单位用量、产出数量、标准用量、'
  + '实际领料、退料、用量差异、损耗率、损耗率上限、单价、是否替代料、替代登记单号）';

const USAGE = `BOM用量与损耗差异核对（免费版） —— 本机执行的AI核对
...
入参写法：
  ...
  · 直接把${MAT}（**含表头**）从 Excel 复制成文本粘进来即可，Tab 分隔最稳

参数：
  -i, --input    入参 JSON 文件路径（也接受直接粘贴的纯文本材料，需含表头）
      --sample   使用内置样例
...
/** 读入参：合法 JSON 就按其结构走；不是 JSON 就当作纯文本材料（例如直接粘贴的领料明细） */
function loadInput(file) {
  ...
  try {
    ...
    return { payload: { text: stripped }, note: '入参文件不是合法 JSON，已按纯文本单据材料处理' };
  }
}

/** 材料不足：说清楚缺什么，并且明确不给结论 */
function reportInsufficient(args, outcome, loaded) {
  const missing = (outcome && outcome.missing) || ['入参无法解析成可核对的单据材料'];
  const advice = (outcome && outcome.advice)
    || `请把${MAT}（含表头）贴进来：可用 {"text": "…"}，或先用 --sample 看看需要什么格式。`;
  ...
  console.log(`（检查项依赖你提供的${MAT}：材料不足或认不出表头时不做任何认定，也不套用默认值。）`);
...
function main() {
  ...
  if (args.input) {
    ...
    console.error(`缺少入参。用 --input 指定${MAT}文件，或用 --sample 自检。`);
    console.error('');
    console.error(USAGE);
    return 1;
  }
  ...
  try {
    ...
    const mark = pathx.join(os.tmpdir(), `skillpay-choice-bom-consumption-variance-check-free${sid ? '-' + sid : ''}.mark`);
...
const HINT_ID = 'bom-consumption-variance-check';
