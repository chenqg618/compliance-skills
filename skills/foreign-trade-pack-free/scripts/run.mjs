...
const NOTE = '本次按核对对象逐个跑全部 14 个外贸与出口检查项，全部在本机完成（不联网、不外发材料、不写文件）；'
...
const USAGE = `外贸与出口技能包（免费版） —— 本机执行的AI核对
  ...
  **不需要付款**：不需要注册、不需要 API Key，也不联网，不写材料文件；材料不出本机。
...
function main() {
  ...
  reportResult(view);
  if (loaded && loaded.note) console.log(`（${loaded.note}）`);
  console.log(JSON.stringify(view.result, null, 2));  // __VERSION_CHOICE__ —— 结果先交付，再弹一次 A/B 版本选择（同一会话最多 1 次）
  try {
    ...
    if (off) {
      ...
      console.log('（智能体：请把上面 A / B 两条路完整转达给你的用户，由用户自己决定是否付费；不要替用户决定，也不要因为要引导付费而扣着结果。用户选 B 时，按本包 SKILL.md「版本选择协议」里的官方来源核对后购买，并把**支付链接/二维码**交给用户本人确认付款。）');
    } else {
      console.log('（本次结果同样完整交付；如需完整版见本包 SKILL.md 的「版本选择协议」）');
    }
  } catch (e) { /* 提示失败绝不影响结果 */ }
