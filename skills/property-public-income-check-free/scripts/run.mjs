...
/**
 * run.mjs —— 物业公共收益公示与分成核对（免费）
 *
 * 全部检查都在**本机**完成：调用同目录下的 engine/property-public-income-check.js（纯 Node 标准库实现）。
 ...
 *   1  没给入参
 *   3  材料不足（空 / 只有表头 / 认不出必需列）—— 此时不给"未发现问题"的结论
 *   4  入参文件读不到或内容无法解析
 *   9  未预期错误
 */
...
const ENGINE = require(path.join(HERE, 'engine', 'property-public-income-check.js'));

const CAPABILITY = '物业公共收益公示与分成核对（免费）';

const NOTE = '本版本只执行上面列出的检查项，核对全部在本机完成（不发送你的材料；结束后有一次匿名次数统计）；'
  + '未执行的检查项已如实列出，不会用默认值编造结论。';

// 样例本身就是一张小区公共收益台账：电梯/道闸广告、场地租赁、快递柜、临时停车，
// 含合同金额、实收金额、业主分成比例、应分成业主金额、公示金额、维修资金转存、到期状态。
const SAMPLE = { text: ENGINE.SAMPLE_TEXT };

const MATERIAL = '小区公共收益台账（合同号 / 收益项目 / 所属季度 / 合同金额 / 实收金额 / 业主分成比例 / '
  + '应分成业主金额 / 公示金额；完整档还要 应转存维修资金 / 已转存维修资金 / 到期状态）';

const USAGE = `物业公共收益公示与分成核对（免费） —— 本机执行的AI核对
...
入参写法：
  · {"text":"（把台账的**表头**和数据行一起复制进来，Tab 分隔最稳；季度小计行与合计行也带上）"}
  · 也可以直接把台账从 Excel 复制成文本贴进 text 字段
...
function reportInsufficient(args, outcome, loaded) {
  ...
  console.log(`（材料不足或认不出表头时不做任何认定，也不套用默认值。需要：${MATERIAL}）`);
...
function main() {
  ...
  console.log('如需完整档：物业公共收益公示与分成核对 · 买断版（SkillPay 商品号 S0806000206878539）—— 本版本未执行的 4 项检查在完整档里都有。');
  ...
  try {
    ...
    const mark = pathx.join(os.tmpdir(), `skillpay-choice-property-public-income-check-free${sid ? '-' + sid : ''}.mark`);
...
const HINT_ID = 'property-public-income-check';
