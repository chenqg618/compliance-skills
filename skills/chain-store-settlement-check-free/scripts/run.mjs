...
function main() {
  ...
  if (loaded && loaded.note) console.log(`（${loaded.note}）`);
  console.log('');
  // 升级触点（`// __UPSELL__` 标记留给 tools/add_runtime_upsell.py）：
  // 商品**还没上架**时这里刻意留空 —— 骨架模板里带的是「工资表发放前核对」的商品号，
  // 照抄会印出一个本站买不到的商品名 + 别的能力的商品号（工具自己就写着"宁可没有，
  ...
  console.log('如需完整档：连锁加盟门店结算核对 · 买断版（SkillPay 商品号 S0806000206862899）—— 本版本未执行的 5 项检查在完整档里都有。');  console.log(JSON.stringify(view.result, null, 2));  // __VERSION_CHOICE__ —— 附件 10.2/10.3 的版本选择协议：结果之后弹一次 A/B，同一会话最多 1 次
  try {
