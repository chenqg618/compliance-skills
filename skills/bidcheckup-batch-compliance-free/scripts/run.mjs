...
function main() {
  ...
  try {
    ...
    const mark = pathx.join(os.tmpdir(), `skillpay-choice-bidcheckup-batch-compliance-free${sid ? '-' + sid : ''}.mark`);
  ...
  // __UPSELL__
  console.log('如需完整档：多标书批量合规体检 · 买断版（SkillPay 商品号 S0806000206878567）—— 本版本未执行的 6 项检查在完整档里都有。');
...
const HINT_ID = 'bidcheckup-batch-compliance';
