...
/**
 * run.mjs —— 工程与项目结算技能包（免费版）
 *
 * 全部检查都在**本机**完成：调用同目录下的 engine/project-settlement-pack.js（纯 Node 标准库实现）。
 * 那个引擎内部装的是仓库里**已有的 14 个工程与项目结算检查引擎**（逐字节拷贝，规则一行没改），
 * 本包只做"项目目录 → 逐项目跑 14 项检查 → 每个项目一行结论"这件编排工作。
 *
 * 刻意不做的事：
 ...
 *
 * 用法：
 *   node scripts/run.mjs --sample                  # 单项目样例（内置，14 项检查全跑）
 *   node scripts/run.mjs --sample-batch            # 3 个项目的批量样例（干净 / 有硬伤 / 未交材料）
 *   node scripts/run.mjs --input 项目目录           # 批量：每个子目录 = 一个项目/一套材料
...
const ENGINE = require(path.join(HERE, 'engine', 'project-settlement-pack.js'));

const CAPABILITY = '工程与项目结算技能包（免费版）';

const NOTE = '本次按项目逐个跑全部 14 个工程与项目结算检查项，全部在本机完成（核对过程不外发材料、不写文件）；'
...
/** `--input <dir>`：每个子目录 = 一个项目；没有子目录时每个文件 = 一个项目。 */
function loadDir(dir) {
  ...
  return {
    payload: { objects },
    note: `目录 ${dir}：按${subdirs.length ? '子目录' : '文件'}识别到 ${objects.length} 个项目`,
  };
}
...
function reportInsufficient(args, outcome, loaded) {
  ...
  const advice = (outcome && outcome.advice)
    || '请用 --input 指向项目目录（每个子目录 = 一个项目），或先用 --sample / --sample-batch 看看需要什么格式。';
  ...
  console.log('（检查项依赖你提供的项目材料：一个项目的材料都没有、或认不出任何一张表时不做任何认定，也不套用默认值。）');
...
/** 每个项目一行的结论：项目名 / 各项通过或有问题 / 材料不足标「未执行」 */
...
function reportResult(view) {
  ...
  console.log(`本次执行的检查项（${view.checks_given.length} 个工程与项目结算检查项）：${view.checks_given.join('、')}`);
...
function main() {
  ...
  try {
    ...
    const mark = pathx.join(os.tmpdir(), `skillpay-choice-project-settlement-pack-free${sid ? '-' + sid : ''}.mark`);
  ...
  // __UPSELL__
  console.log('如需完整档：工程与项目结算技能包 · 买断版（SkillPay 商品号 S0806000206877487）—— 本版本未执行的 4 项检查在完整档里都有。');
...
const HINT_ID = 'project-settlement-pack';
