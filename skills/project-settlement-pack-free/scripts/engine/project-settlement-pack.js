'use strict';
/**
 * project-settlement-pack-full.js —— 工程与项目结算技能包（免费档 / 完整档共用源码）
 *
 * 谁在什么时候必须用它：**工程公司的项目会计 / 成本岗 / 代建与业主方的工程管理岗，
 * 在每期计量报量、分包结算、开票回款、竣工转固这些固定节点上**。要核的从来不是一张表，
 * 而是一整套互相关联的工程结算底稿：本期产值与计量、进度款与质保金扣留、质保金退还、
 * 项目开票与回款、挣值（EVM）、在建工程转固与利息资本化、维修资金专项付款、分包结算、
 * 预付款冲抵、违约金与索赔结算、安全生产费提取使用、投标保证金收退、软件外包里程碑付款 ——
 * 每张表都有自己的勾稽关系，**逐张手核既慢又容易漏**，而且每个项目、每一期都要重复一遍；
 * 少扣一笔质保金、多付一笔进度款、转固金额算错，后果都比多花半小时严重得多。
 *
 * 这个包做的事**只有一件**（不是重写任何规则）：把仓库里**已有的 14 个工程与项目结算检查引擎**
 * 装进一个批量壳里，`--input <目录>`（每个子目录 = 一个项目 / 一套材料）一次跑完所有项目，
 * **每个项目一行结论**。
 *   规则来源：`scripts/engine/parts/<检查项>/`（**逐字节拷贝**自那 14 个免费包，一行都没改；
 *   本包只做"目录 → 项目 → 逐项派发"的编排与汇总）。
 * * ⚠️ 本文件是 **免费档子集**：只实现免费检查项；**完整档（付费）的实现不在这个包里**。
 * `CHECKS_WITHHELD` 只是"未执行的检查项"的**说明文本**，不是实现。
 *
 * 契约（与仓库其它引擎一致，别改名字）：
 *   run(payload) -> {status:'success', result} | {status:'insufficient_input', missing, advice}
 *     payload.text            单项目：该项目的一套工程结算材料文本（每项用 `=== 检查项 ===` 分段）
 *     payload.objects[]       多项目：{name, files:[{name, text}]} —— 每个子目录 = 一个项目
 ...
 * ⚠️ 材料不足**绝不给结论**：某个项目没材料就单独标"未执行"，一个项目都跑不了就整体不给结论。
 * ⚠️ 本工具**不代替审计、不出鉴证意见**，也不判断某项工程款该不该付、结算该按什么口径审定。
 */

const MEMBERS = [
  {
    label: '工程产值与进度确认核对',
    id: 'construction-output-value-check',
    entry: require('./parts/construction-output-value-check/construction-output-value-check.js'),
  },
  {
    label: '工程进度款与质保金核算核对',
    id: 'progress-payment-check',
    entry: require('./parts/progress-payment-check/progress-payment-check.js'),
  },
  {
    label: '工程质量保证金扣留与退还核对',
    id: 'retention-money-check',
    entry: require('./parts/retention-money-check/retention-money-check.js'),
  },
  {
    label: '项目开票与回款核对',
    id: 'project-billing-collection-check',
    entry: require('./parts/project-billing-collection-check/project-billing-collection-check.js'),
  },
  {
    label: '项目挣值分析（EVM）核对',
    id: 'project-evm-check',
    entry: require('./parts/project-evm-check/project-evm-check.js'),
  },
  {
    label: '在建工程转固与利息资本化核对',
    id: 'cip-capitalization-check',
    entry: require('./parts/cip-capitalization-check/cip-capitalization-check.js'),
  },
  {
    label: '在建工程转固核对',
    id: 'cip-transfer-check',
    entry: require('./parts/cip-transfer-check/cip-transfer-check.js'),
  },
  {
    label: '维修资金与专项工程付款核对',
    id: 'maintenance-fund-payment-check',
    entry: require('./parts/maintenance-fund-payment-check/maintenance-fund-payment-check.js'),
  },
  {
    label: '分包结算与产值核对',
  ...
  {
    label: '软件外包里程碑验收与付款核对',
    id: 'software-outsource-milestone-check',
    entry: require('./parts/software-outsource-milestone-check/software-outsource-milestone-check.js'),
  },
];

/* 免费档执行：14 个工程与项目结算检查项，逐个项目全跑一遍（免费层的核心产出：一次跑完所有项目） */
const CHECKS_GIVEN = MEMBERS.map((m) => m.label);

/* 完整档追加（**多一种能力**，不是多几个数）：跨项目汇总台账 —— 单项目结果里根本不存在的东西 */
...
const OUT_OF_SCOPE = [
  '判断某项工程款该不该付、结算金额该不该审定（属于合同履约与工程造价判断，请按合同约定与造价咨询/监理签证结论执行）',
  '判断定额套用、取费标准、工程量计算规则该按哪一版执行（各地各行业口径不同，请以合同约定的计价依据为准）',
  '代替审计程序，或出具鉴证意见与审计意见（只做表内/表间的算术与勾稽核对）',
  '核对签证单、监理报告、发票、银行回单、保证金收据的真伪，或比对业主/监理/税务系统的逐行明细',
  '读取工程造价软件 / ERP / 资金系统的导出文件（需要你先导出成文本，每个项目一个目录）',
  '判断挣值（EVM）的基准计划是否合理、形象进度百分比填得准不准（本工具只核你给的表内数字与表间勾稽关系）',
];
...
/* 有一处硬伤的样例：把「工程产值与进度确认核对」里 2026-01 那期的**本期计量金额**从 980000.00
   改成 970000.00（本期已完产值 1000000.00 − 本期扣款 20000.00 = 980000.00，与 970000 不符）
   ⇒ 必须报出来，并带**原文行号**。 */
const DIRTY_FROM = '滨江路改造工程\t2026-01\t10%\t10000000.00\t1000000.00\t1000000.00\t980000.00';
const DIRTY_TO = '滨江路改造工程\t2026-01\t10%\t10000000.00\t1000000.00\t1000000.00\t970000.00';
const DIRTY_TEXT = SAMPLE_TEXT.replace(DIRTY_FROM, DIRTY_TO);

/* 只覆盖 3 项的样例：用来演示"材料只覆盖了一部分"时，批量壳**明确报一条**，
   而不是把"跑了 3 项"说成"14 项都核过了"。 */
const PARTIAL_TITLES = ['工程产值与进度确认核对', '工程质量保证金扣留与退还核对', '分包结算与产值核对'];
const PARTIAL_TEXT = SAMPLE_TEXT.split(/(?=^=== )/m)
  .filter((s) => PARTIAL_TITLES.some((t) => s.trim().indexOf(`=== ${t} ===`) === 0))
  .join('\n');

/* 批量样例：3 个项目（干净 / 有一处硬伤 / 没交材料），用来演示"一次跑完所有项目"。 */
const SAMPLE_OBJECTS = [
  { name: '滨江路改造工程（2026-03 结算）', files: [{ name: '工程结算材料.txt', text: SAMPLE_TEXT }] },
  { name: '城东安置房项目（2026-03 结算）', files: [{ name: '工程结算材料.txt', text: DIRTY_TEXT }] },
  { name: '缺料项目（只登记未交材料）', files: [] },
];
...
/** 入参归一化成"项目列表"：`{text}` 是单项目，`{objects:[…]}`（或 `{projects:[…]}`）是批量。 */
function normalizeObjects(payload) {
  ...
  if (list) {
    for (const o of list) {
      if (!o) continue;
      raw.push({ name: String(o.name || o.object || o.project || '').trim(), files: coerceFiles(o) });
    }
  }
  if (payload.text !== undefined || payload.content !== undefined) {
    raw.push({
      name: String(payload.object_name || payload.object || payload.project_name || payload.project
        || payload.name || '单项目').trim(),
      files: coerceFiles(payload),
    });
  }
  const out = [];
  raw.forEach((o, i) => out.push({ name: o.name || `项目${i + 1}`, files: o.files }));
  return out;
}

/** 覆盖缺口检查：项目交了材料，但只覆盖了一部分检查项 ⇒ **明确报一条**，
 *  不能因为"跑了几项"就以为 14 项都核过了。整份材料都没交的项目不在这里报（那是项目级的"未执行"）。 */
function checkClientCoverage(it) {
  ...
  return {
    ...
    line: 1,
    message: `项目「${it.client}」的材料只覆盖了 ${ran.length} / ${it.checks.length} 个工程与项目结算检查项，`
...
/** 一个项目跑完全部 14 项；每一项的实际规则在 parts/ 里（本文件一行规则都没改）。
 *  派发方式：把该项目的每一段材料依次喂给成员引擎，**第一个跑通的段**就是这一项的表；
...
function runOneObject(object) {
  ...
  if (!candidates.length || materialLines < 5) {
    return {
      ...
      reason: '没有收到这个项目的可用材料（材料为空或只有几行）',
    };
  }
  ...
  for (const m of MEMBERS) {
    ...
    if (!hit) {
      ...
      checks.push({
        ...
        missing: bestMissing || ['这个项目的材料里没有这一项需要的表头行'],
      });
      continue;
    }
  ...
  if (cover) {
    findings.push(Object.assign({}, cover, {
      ...
      source_file: '', source_line: 0, evidence: '项目级',
    }));
  }
  ...
  /* 这一行有没有被真正核对过（有材料、且至少有一项能跑）⇒ 才够格进跨项目汇总台账。
     免费档与完整档都用它：台账只收"真跑过"的项目行，没交材料的项目不进总表。 */
...
function run(payload) {
  ...
  if (!payload) {
    return insufficient([
      '一个项目的材料都没收到（objects[] 与 text 都是空）',
      '单项目用 {"text":"…"}；批量用 {"objects":[{"name":"项目名","files":[{"name":"材料.txt","text":"…"}]}]}',
    ]);
  }
  ...
  if (!objects.length) {
    return insufficient([
      '一个项目的材料都没收到（objects[] 与 text 都是空）',
      '每个项目一个子目录，目录里放该项目的一套工程结算材料（每项用 `=== 检查项 ===` 分段，Tab 分隔最稳）',
    ]);
  }
  ...
  if (!usable.length) {
    return insufficient(
      [`全部 ${rows.length} 个项目都没有可用材料（每个项目目录里要有该项目的工程结算材料，且每张表要有表头行）`]
        .concat(rows.map((r) => `项目「${r.object}」：没有可用材料`)),
      '把每个项目的工程结算材料放进各自子目录后再跑；材料不足时本工具不做任何认定、也不给任何结论。');
  }
  ...
  const result = {
    service_type: 'PROJECT_SETTLEMENT_PACK',
    scope: {
    ...
    note: `本次对 ${rows.length} 个项目逐个跑了 ${CHECKS_GIVEN.length} 个工程与项目结算检查项（每个成员包内部还有 `
      + `${TOTAL_SUB_CHECKS} 项子检查）；未执行的检查项见 scope.checks_not_run 与 `
      + 'scope.sub_checks_not_run。',
    disclaimer: '只把各项目材料里的表内/表间算术与勾稽核一遍，结论都带**原文文件与行号**、可由第三方复算；'
      + '**不代替审计、不出具鉴证意见**，也不判断某项工程款该不该付、结算金额该不该审定。',
  };

  /* 分层：
     · 免费档（免费包里就是下面这两行 + 再下面那句 return，付费块被整块摘掉）：
       只报**实际执行**的那 14 项，未执行项如实列出（说明文本，不是实现）。
     · 完整档（本文件）：在上面那层之上再挂一层跨项目汇总台账
       （项目 × 检查项总表、结算风险排序清单、跨项目共性问题归类、Markdown / CSV 导出）。 */
