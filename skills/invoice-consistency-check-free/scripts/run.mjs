...
/** 内置样例：与 templates/sample.json 完全等价（先读文件，读不到才用内置文本） */
function samplePayload() {
  try {
    const raw = fs.readFileSync(path.join(HERE, '..', 'templates', 'sample.json'), 'utf8').replace(/^\uFEFF/, '');
...
const USAGE = `票据一致性AI核对（免费） —— 本机执行的AI核对
  ...
  **不需要付款**：不需要注册、不需要任何 API Key，材料不出本机（运行结束后有一次匿名次数统计，可用 --no-stats 关闭）。
...
参数：
  -i, --input    入参 JSON 文件路径（也接受直接粘贴票据全文的纯文本文件）
      --sample   使用内置样例
...
/** 读入参：合法 JSON 就按其结构走；不是 JSON 就当作纯文本材料（例如直接粘贴的票据全文） */
function loadInput(file) {
  ...
  try {
    ...
    return { payload: { text: stripped }, note: '入参文件不是合法 JSON，已按纯文本票据材料处理' };
  }
}

/** 材料不足：说清楚缺什么，并且明确不给结论 */
function reportInsufficient(args, outcome, loaded) {
  const missing = (outcome && outcome.missing) || ['入参无法解析成可核对的票据材料'];
  const advice = (outcome && outcome.advice)
    || '请提供票据文本（text 或 content），或先用 --sample 看看需要什么格式。';
  ...
  console.log('（检查项依赖你提供的票据文本：材料不足时不做任何认定，也不套用默认值。）');
...
function main() {
  ...
  if (args.input) {
    ...
    console.error('缺少入参。用 --input 指定票据文件，或用 --sample 自检。');
    console.error('');
    console.error(USAGE);
    return 1;
  }
  ...
  if (loaded && loaded.note) console.log(`（${loaded.note}）`);
  console.log('');
  const byCat = (view.result.summary && view.result.summary.by_category) || {};
  console.log(`按检查项统计（共 ${view.result.summary.total} 条，P0 ${view.result.summary.p0} / P1 ${view.result.summary.p1} / P2 ${view.result.summary.p2}）：`);
  for (const c of view.checks_given) {
  ...
  try {
    ...
    const mark = pathx.join(os.tmpdir(), `skillpay-choice-invoice-consistency-check-free${sid ? '-' + sid : ''}.mark`);
    ...
    if (off) {
      ...
      console.log('');
      console.log('本次为【免费版】：上面就是免费版的完整结果（已执行全部免费检查项）；另有 9 项完整版会补上（差在哪见上）。');
  ...
  // __UPSELL__
  console.log('如需完整档：票据一致性AI核对 · 买断版（SkillPay 商品号 S0806000206878581）—— 本版本未执行的 9 项检查在完整档里都有。');
...
const HINT_ID = 'invoice-consistency-check';
