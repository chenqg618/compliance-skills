...
/**
 * run.mjs —— 采购与付款技能包（免费版）
 *
 * 全部检查都在**本机**完成：调用同目录下的 engine/procurement-payable-pack.js（纯 Node 标准库实现）。
 * 那个引擎内部装的是仓库里**已有的 14 个采购与付款检查引擎**（逐字节拷贝，规则一行没改），
 * 本包只做"对象目录 → 逐个对象跑 14 项检查 → 每个对象一行结论"这件编排工作。
 *
 * 刻意不做的事：
 ...
 *   node scripts/run.mjs --sample-batch            # 3 个对象的批量样例（样例稿 / 有硬伤 / 未交材料）
 *   node scripts/run.mjs --input 对象目录           # 批量：每个子目录 = 一个对象/一套材料
 ...
 *     （目录里没有子目录时，直接把每个文件当一个对象）
 *   也可以把 14 张表拆成 14 个文件放进同一个子目录 —— 引擎会按 `=== 检查项 ===` 段标题认表。
 *
 * 退出码：
...
const ENGINE = require(path.join(HERE, 'engine', 'procurement-payable-pack.js'));

const CAPABILITY = '采购与付款技能包（免费版）';

const NOTE = '本次按对象逐个跑全部 14 个采购与付款检查项，全部在本机完成（核对过程不外发材料、不写文件）；'
  + '未执行的检查项已如实列出，不会用默认值编造结论。';

const TEXT_EXT = new Set(['.txt', '.tsv', '.csv', '.md', '.text', '.tab', '.log', '.dat', '.json']);

const SAMPLE = { text: ENGINE.SAMPLE_TEXT, object_name: '样例对象（单对象模式）' };
const SAMPLE_BATCH = { objects: ENGINE.SAMPLE_OBJECTS };

const USAGE = `采购与付款技能包（免费版） —— 本机执行的AI核对

  **不需要付款**：不需要注册、不需要 API Key，核对过程在本机完成，不写文件；材料不出本机。
  检查项（每个对象都跑这几项）：${ENGINE.CHECKS_GIVEN.join('、')}
...
入参写法：
  · 批量（推荐）：--input 指向一个目录，里面每个子目录就是一个对象，子目录里放该对象的一套采购与付款材料
...
/** `--input <dir>`：每个子目录 = 一个对象；没有子目录时每个文件 = 一个对象。 */
function loadDir(dir) {
  ...
  return {
    payload: { objects },
    note: `目录 ${dir}：按${subdirs.length ? '子目录' : '文件'}识别到 ${objects.length} 个对象`,
  };
}
...
function reportInsufficient(args, outcome, loaded) {
  const missing = (outcome && outcome.missing) || ['入参无法解析成可核对的对象材料'];
  const advice = (outcome && outcome.advice)
    || '请用 --input 指向对象目录（每个子目录 = 一个对象），或先用 --sample / --sample-batch 看看需要什么格式。';
  ...
  console.log('（检查项依赖你提供的对象材料：一个对象的材料都没有、或认不出任何一张表时不做任何认定，也不套用默认值。）');
...
/** 每个对象一行的结论：对象名 / 各项通过或有问题 / 材料不足标「未执行」 */
...
function reportResult(view) {
  const r = view.result;
  console.log('免费AI核对完成（批量壳：一次跑完所有对象）');
  console.log(`本次执行的检查项（${view.checks_given.length} 个采购与付款检查项）：${view.checks_given.join('、')}`);
  ...
  console.log('');
  console.log(`对象数 ${r.summary.objects}｜有问题 ${r.summary.objects_with_issues}｜干净 ${r.summary.objects_clean}｜未执行（没交材料）${r.summary.objects_not_run}`);
  console.log('');
  console.log('对象 × 结论（每个对象一行）');
  for (const o of r.objects) console.log(objectLine(o));
  if (r.findings.length) {
    console.log('');
    console.log(`问题明细（共 ${r.findings.length} 条，带对象 + 原文文件与行号）`);
...
function main() {
  ...
  try {
    ...
    const mark = pathx.join(os.tmpdir(), `skillpay-choice-procurement-payable-pack-free${sid ? '-' + sid : ''}.mark`);
  ...
  // __UPSELL__
  console.log('如需完整档：采购与付款技能包 · 买断版（SkillPay 商品号 S0806000206877491）—— 本版本未执行的 4 项检查在完整档里都有。');
...
const HINT_ID = 'procurement-payable-pack';
