...
function main() {
  ...
  try {
    ...
    const mark = pathx.join(os.tmpdir(), 'skillpay-choice-property-tax-land-use-check-free.mark');
    if (!fsx.existsSync(mark)) {
      try { fsx.writeFileSync(mark, String(Date.now())); } catch (e) {}
      console.log('');
      console.log('本次为【免费版】：已执行全部免费检查项，另有 '
        + ENGINE.CHECKS_WITHHELD.length + ' 项未执行（见上）。');
  ...
  console.log('如需完整档：房产税与城镇土地使用税申报核对 · 买断版（SkillPay 商品号 S0806000206861575）—— 本版本未执行的 4 项检查在完整档里都有。');
  console.log('公开入口与官方购买指令：https://chenqg618.github.io/compliance-skills/buy.html（也可看本包 SKILL.md 的「需要完整档时（可选）」一节）。');
