...
/**
 * run.mjs —— 废料边角料销售与回收结算核对（免费版）
 *
 * 全部检查都在**本机**完成：调用同目录下的 engine/scrap-sale-check.js（纯 Node 标准库实现）。
 ...
 *   node scripts/run.mjs --input your-ledger.json
 *   node scripts/run.mjs --input your-ledger.json --json
 *
 * 退出码：
...
const ENGINE = require(path.join(HERE, 'engine', 'scrap-sale-check.js'));

const CAPABILITY = '废料边角料销售与回收结算核对（免费版）';

const NOTE = '本版本只执行上面列出的检查项，核对全部在本机完成（不发送你的材料；结束后有一次匿名次数统计）；'
  + '未执行的检查项已如实列出，不会用默认值编造结论。';

const SAMPLE = { text: ENGINE.SAMPLE_TEXT };   // 样例本身就是一张带表头的废料边角料销售与回收结算台账（过磅净重、扣杂量、水分扣量、结算数量与单价、结存数量、台账收入）

const USAGE = `废料边角料销售与回收结算核对（免费版） —— 本机执行的AI核对
...
入参写法：
  ...
  · 直接把废料边角料销售与回收结算台账（过磅净重、扣杂量、水分扣量、结算数量与单价、结存数量、台账收入）（**含表头**）从 Excel 复制成文本粘进来即可，Tab 分隔最稳

参数：
  -i, --input    入参 JSON 文件路径（也接受直接粘贴的废料边角料销售与回收结算台账（过磅净重、扣杂量、水分扣量、结算数量与单价、结存数量、台账收入）纯文本，需含表头）
      --sample   使用内置样例
...
function reportInsufficient(args, outcome, loaded) {
  ...
  const advice = (outcome && outcome.advice)
    || '请把废料边角料销售与回收结算台账（过磅净重、扣杂量、水分扣量、结算数量与单价、结存数量、台账收入）（含表头）贴进来：可用 {"text": "…"}，或先用 --sample 看看需要什么格式。';
  ...
  console.log('（检查项依赖你提供的废料边角料销售与回收结算台账（过磅净重、扣杂量、水分扣量、结算数量与单价、结存数量、台账收入）：材料不足或认不出表头时不做任何认定，也不套用默认值。）');
...
function main() {
  ...
  if (args.input) {
    ...
    console.error('缺少入参。用 --input 指定废料边角料销售与回收结算台账（过磅净重、扣杂量、水分扣量、结算数量与单价、结存数量、台账收入）文件，或用 --sample 自检。');
    console.error('');
    console.error(USAGE);
    return 1;
  }
  ...
  console.log('如需完整档：废料边角料销售与回收结算核对 · 买断版（SkillPay 商品号 S0806000206877417）—— 本版本未执行的 7 项检查在完整档里都有。');
  ...
  try {
    ...
    const mark = pathx.join(os.tmpdir(), `skillpay-choice-scrap-sale-check-free${sid ? '-' + sid : ''}.mark`);
    ...
    if (off) {
      ...
      console.log('');
      console.log('本次为【免费版】：上面就是免费版的完整结果（已执行全部免费检查项）；另有 7 项完整版会补上（差在哪见上）。');
...
const HINT_ID = 'scrap-sale-check';
