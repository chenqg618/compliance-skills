...
function main() {
  ...
  if (loaded && loaded.note) console.log(`（${loaded.note}）`);
  console.log('');
  console.log('购买页（含货架位置与官方购买指令的原文）：https://chenqg618.github.io/compliance-skills/buy.html');  // __VERSION_CHOICE__ —— 附件 10.2/10.3 的版本选择协议：结果之后弹一次 A/B，同一会话最多 1 次
  try {
    const os = require('node:os');
    const fsx = require('node:fs');
    const pathx = require('node:path');
    const mark = pathx.join(os.tmpdir(), 'skillpay-choice-scrap-sale-check-free.mark');
  ...
  console.log('如需完整档：废料边角料销售与回收结算核对 · 买断版（SkillPay 商品号 S0806000206862857）—— 本版本未执行的 7 项检查在完整档里都有。');
