...
/**
 * run.mjs —— 合同一致性AI核对（免费）
 *
 * 全部检查都在**本机**完成：调用同目录下的 engine/contract-consistency.js（纯 Node 标准库实现）。
...
const ENGINE = require(path.join(HERE, 'engine', 'contract-consistency.js'));

const CAPABILITY = '合同一致性AI核对（免费）';
...
const USAGE = `合同一致性AI核对（免费） —— 本机执行的AI核对
...
参数：
  -i, --input    入参 JSON 文件路径（也接受直接粘贴合同全文的纯文本文件）
      --sample   使用内置样例
...
function loadInput(file) {
  ...
  try {
    ...
    return { payload: { text: stripped }, note: '入参文件不是合法 JSON，已按纯文本合同材料处理' };
  }
}

/** 材料不足：说清楚缺什么，并且明确不给结论 */
function reportInsufficient(args, outcome, loaded) {
  const missing = (outcome && outcome.missing) || ['入参无法解析成可校验的合同材料'];
  const advice = (outcome && outcome.advice)
    || '请提供合同全文（text 或 content），或先用 --sample 看看需要什么格式。';
  ...
  console.log('（检查项依赖你提供的合同正文：材料不足时不做任何认定，也不套用默认值。）');
...
function main() {
  ...
  if (args.input) {
    ...
    console.error('缺少入参。用 --input 指定合同文件，或用 --sample 自检。');
    console.error('');
    console.error(USAGE);
    return 1;
  }
  ...
  try {
    ...
    const mark = pathx.join(os.tmpdir(), `skillpay-choice-contract-consistency-check-free${sid ? '-' + sid : ''}.mark`);
...
const HINT_ID = 'contract-consistency-check';
