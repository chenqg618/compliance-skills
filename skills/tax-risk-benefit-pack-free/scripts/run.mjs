...
/**
 * run.mjs —— 税费自查与优惠技能包（免费版）
 *
 * 全部检查都在**本机**完成：调用同目录下的 engine/tax-risk-benefit-pack.js（纯 Node 标准库实现）。
 * 那个引擎内部装的是仓库里**已有的 5 个税费自查检查引擎**（逐字节拷贝，规则一行没改），
 * 本包只做"客户目录 → 逐客户跑这 5 项检查 → **每家客户一行结论**"这件编排工作。
 *
 * 刻意不做的事：
 ...
 *
 * 用法：
 *   node scripts/run.mjs --sample                   # 单客户样例（内置，5 项检查全跑）
 *   node scripts/run.mjs --sample-batch             # 3 家客户的批量样例（干净 / 有硬伤 / 未交材料）
 *   node scripts/run.mjs --input 客户目录            # 批量：每个子目录 = 一家客户/一套材料
 ...
 *     （目录里没有子目录时，直接把每个文件当一家客户）
 *   也可以把 14 张表拆成 5 个文件放进同一个子目录 —— 引擎会自己认表头找对应的那一项。
 *
 * 退出码：
 *   0  已执行检查（结果里有问题项或没有问题项都算执行成功）
 *   1  没给入参
 *   3  材料不足（一家客户的材料都没有 / 空 / 只有空白 / 认不出任何一张表）—— 此时不给任何结论
 *   4  入参文件或目录读不到 / 内容无法解析
 *   9  未预期错误
 */
...
const ENGINE = require(path.join(HERE, 'engine', 'tax-risk-benefit-pack.js'));

const CAPABILITY = '税费自查与优惠技能包（免费版）';

const NOTE = '本次按客户逐个跑全部 5 个税费自查检查项，全部在本机完成（核对过程不外发材料、不写文件）；'
...
/** `--input <dir>`：每个子目录 = 一家客户；没有子目录时每个文件 = 一家客户。 */
function loadDir(dir) {
  ...
  return {
    payload: { objects },
    note: `目录 ${dir}：按${subdirs.length ? '子目录' : '文件'}识别到 ${objects.length} 家客户`,
  };
}
...
function reportInsufficient(args, outcome, loaded) {
  ...
  const advice = (outcome && outcome.advice)
    || '请用 --input 指向客户目录（每个子目录 = 一家客户），或先用 --sample / --sample-batch 看看需要什么格式。';
  ...
  console.log('（检查项依赖你提供的客户材料：一家客户的材料都没有、或认不出任何一张表时不做任何认定，也不套用默认值。）');
...
/** 每家客户一行的结论：客户名 / 各项通过或有问题 / 材料不足标「未执行」 */
...
function reportResult(view) {
  ...
  console.log(`本次执行的检查项（${view.checks_given.length} 个税费自查检查项）：${view.checks_given.join('、')}`);
...
function main() {
  ...
  try {
    ...
    const mark = pathx.join(os.tmpdir(), `skillpay-choice-tax-risk-benefit-pack-free${sid ? '-' + sid : ''}.mark`);
  ...
  // __UPSELL__
  console.log('如需完整档：税费自查与优惠技能包 · 买断版（SkillPay 商品号 S0806000206881930）—— 本版本未执行的 4 项检查在完整档里都有。');
...
const HINT_ID = 'tax-risk-benefit-pack';
