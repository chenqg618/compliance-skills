...
const NOTE = '本次按客户逐个跑全部 5 个税费自查检查项，全部在本机完成（不联网、不外发材料、不写文件）；'
...
function main() {
  ...
  try {
    ...
    if (off) {
      ...
      console.log('');
      console.log('本次为【免费版】：上面就是免费版的完整结果（5 个税费自查检查项 × 全部客户；未执行项已如实列出）。');
      console.log('买断版另加一层能力：跨客户汇总台账（客户 × 检查项总表、税费自查风险排序清单、共性问题归类、逐客户处理项、Markdown/CSV 导出）。');
