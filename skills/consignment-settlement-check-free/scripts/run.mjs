...
/**
 * run.mjs —— 寄售代销结算核对（免费版）
 *
 * 全部检查都在**本机**完成：调用同目录下的 engine/consignment-settlement-check-full.js（纯 Node 标准库实现）。
 * 检查全部在**本机**完成（纯 Node 标准库），不读环境变量、不外发材料、不需要注册与 API Key。
 * 运行结束后会向服务端做**一次匿名次数统计**（只发包标识与一个不含材料的匿名安装标识）；
 * 前 3 次服务端不返回任何内容，**第 4 次起**若服务端返回完整档说明才会打印一次；
 * 统计失败/离线/超时 ⇒ 什么都不做，核对结果与退出码完全不受影响。
 *
 * 刻意不做的事：
 ...
 *   node scripts/run.mjs --input my-consignment.json
 *   node scripts/run.mjs --input my-consignment.json --json
 *   node scripts/batch.mjs --input 多期台账.txt        # （免费版不含批量入口）
 *
 * 退出码：
...
const ENGINE = require(path.join(HERE, 'engine', 'consignment-settlement-check.js'));

const CAPABILITY = '寄售代销结算核对（免费版）';

const NOTE = '本版本只执行上面列出的检查项，核对全部在本机完成（不发送你的材料；结束后有一次匿名次数统计）；'
  + '未执行的检查项已如实列出，不会用默认值编造结论。';

const SAMPLE = { text: ENGINE.SAMPLE_TEXT };

const USAGE = `寄售代销结算核对（免费版） —— 本机执行的AI核对

  **不需要付款**：不需要注册、不需要 API Key，材料不出本机（运行结束后有一次匿名次数统计，可用 --no-stats 关闭）。
  检查项：${ENGINE.CHECKS_GIVEN.join('、')}
  完整档 = 免费层的 7 项 + 追加的 5 项判定（已销未结、手续费率高于合同约定、退货未冲减、
  寄售库存与总账不符、代销方结存与实盘差），并按差额金额输出追收处理清单。

用法：
  node scripts/run.mjs --sample
  node scripts/run.mjs --input my-consignment.json
  node scripts/batch.mjs --input 多期台账.txt          # 批量：各段用一行 === 段名 === 分隔

入参写法：
  · {"text":"（把你那张**寄售/代销台账**的表头和若干行一起复制进来，Tab 分隔最稳）"}
  · 直接把台账（清单号、代销方、商品编码、期初结存、本期发货、本期已销数量、退货数量、含税单价、
    代销方已销金额、合同手续费率、代销手续费率、代销手续费、应结金额、已结金额、总账寄售库存、
    代销方实盘结存）从 Excel 复制成文本粘进来即可，Tab 分隔最稳

参数：
  -i, --input    入参 JSON 文件路径（也接受直接粘贴的台账纯文本，需含表头）
      --sample   使用内置样例
...
function loadInput(file) {
  ...
  try {
    ...
    return { payload: { text: stripped }, note: '入参文件不是合法 JSON，已按纯文本台账材料处理' };
  }
}

/** 材料不足：说清楚缺什么，并且明确不给结论 */
function reportInsufficient(args, outcome, loaded) {
  const missing = (outcome && outcome.missing) || ['入参无法解析成可核对的寄售台账'];
  const advice = (outcome && outcome.advice)
    || '请把寄售/代销台账（含表头）贴进来：可用 {"text": "…"}，或先用 --sample 看看需要什么格式。';
  ...
  console.log('（检查项依赖你提供的寄售/代销台账：材料不足或认不出表头时不做任何认定，也不套用默认值。）');
...
function main() {
  ...
  if (args.input) {
    ...
    console.error('缺少入参。用 --input 指定寄售/代销台账文件，或用 --sample 自检。');
    console.error('');
    console.error(USAGE);
    return 1;
  }
  ...
  let outcome;
  try {
    // 免费版：只执行本版本范围内的检查项（未执行项已如实列出，绝不编造结论）。
    // 少了这一行，引擎里的 paid 开关不会打开，买家会只拿到免费层的结果。
  ...
  console.log('如需完整档：寄售代销结算核对 · 买断版（SkillPay 商品号 S0806000206866030）—— 本版本未执行的 5 项检查在完整档里都有。');
  ...
  try {
    ...
    const mark = pathx.join(os.tmpdir(), `skillpay-choice-consignment-settlement-check-free${sid ? '-' + sid : ''}.mark`);
    ...
    if (off) {
      ...
      console.log('');
      console.log('本次为【免费版】：上面就是免费版的完整结果（已执行全部免费检查项）；另有 5 项完整版会补上（差在哪见上）。');
...
const HINT_ID = 'consignment-settlement-check';
