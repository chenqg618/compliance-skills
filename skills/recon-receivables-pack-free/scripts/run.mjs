...
/**
 * run.mjs —— 对账与往来款技能包（免费版）
 *
 * 全部检查都在**本机**完成：调用同目录下的 engine/recon-receivables-pack.js（纯 Node 标准库实现）。
 * 那个引擎内部装的是仓库里**已有的 14 个对账与往来款检查引擎**（逐字节拷贝，规则一行没改），
 * 本包只做"客户目录 → 逐客户跑 14 项往来检查 → 每个客户一行结论"这件编排工作。
 *
 * 刻意不做的事：
...
const ENGINE = require(path.join(HERE, 'engine', 'recon-receivables-pack.js'));

const CAPABILITY = '对账与往来款技能包（免费版）';

const NOTE = '本次按客户逐个跑全部 14 个对账与往来款检查项，全部在本机完成（核对过程不外发材料、不写文件）；'
...
const USAGE = `对账与往来款技能包（免费版） —— 本机执行的AI核对
...
入参写法：
  · 批量（推荐）：--input 指向一个目录，里面每个子目录就是一个客户，子目录里放该客户的一套往来材料
...
function reportResult(view) {
  ...
  console.log(`本次执行的检查项（${view.checks_given.length} 个对账与往来款检查项）：${view.checks_given.join('、')}`);
...
function main() {
  ...
  try {
    ...
    const mark = pathx.join(os.tmpdir(), `skillpay-choice-recon-receivables-pack-free${sid ? '-' + sid : ''}.mark`);
  ...
  // __UPSELL__
  console.log('如需完整档：对账与往来款技能包 · 买断版（SkillPay 商品号 S0806000206881913）—— 本版本未执行的 4 项检查在完整档里都有。');
...
const HINT_ID = 'recon-receivables-pack';
