...
const NOTE = '本次按机构逐个跑全部 14 个行业专项检查项，全部在本机完成（不联网、不外发材料、不写文件）；'
...
function main() {
  ...
  try {
    ...
    if (off) {
      ...
      console.log('');
      console.log('本次为【免费版】：上面就是免费版的完整结果（14 个行业专项检查项 × 全部机构；未执行项已如实列出）。');
      console.log('买断版另加一层能力：跨机构汇总台账（机构 × 检查项总表、行业风险排序清单、共性问题归类、逐机构处理项、Markdown/CSV 导出）。');
