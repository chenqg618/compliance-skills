...
function main() {
  ...
  console.log('如需完整档：现金流预测与实际差异核对 · 买断版（SkillPay 商品号 S0806000206853065）—— 本版本未执行的 5 项检查在完整档里都有。');
  console.log('公开入口与官方购买指令：https://chenqg618.github.io/compliance-skills/buy.html（也可看本包 SKILL.md 的「需要完整档时（可选）」一节）。');
  ...
  try {
    ...
    const mark = pathx.join(os.tmpdir(), 'skillpay-choice-cash-flow-variance-check-free.mark');
