...
version: 1.1.0
summary: 把一个项目的投标材料先做一轮免费的机械体检，报价算术 + 模板占位符，不需要付款，也不需要注册。 免费、无需注册、无需 API Key。
...
description: 把一个项目的投标材料先做一轮免费的机械体检，报价算术 + 模板占位符，不需要付款，也不需要注册。 本免费版执行 逐家报价算术校验、模板占位符扫描。触发词包括 招投标全案合规审计、逐家报价算术校验、模板占位符扫描。触发词还包括投标文件检查、标书审查、投标文件审查、标书合规检查。
description_zh: 把一个项目的投标材料先做一轮免费的机械体检，报价算术 + 模板占位符，不需要付款，也不需要注册。 本免费版执行 逐家报价算术校验、模板占位符扫描。
...
> 一句话：把一个项目的投标材料先做一轮免费的机械体检：报价算术 + 模板占位符，不需要付款，也不需要注册。
...
```bash
curl -s -X POST https://www.tokendidi.cn/api/v1/tender-compliance-audit/free \
  -H 'Content-Type: application/json' \
  -d '{"bidders": [{"name": "河南甲建设有限公司", "price": 1000000, "items": [{"name": "钢筋制安", "qty": 10, "price": 100, "amount": 900}], "text": "投标报价壹佰万元整 法定代表人：王五 盖章 承诺【填写：项目名称】"}]}'
```

## 入参

给一个 bidders 数组（1 至 50 家）。其余字段本版本不使用。
...
只做机械层面的缺陷扫描，不构成法律意见，也不替代评标委员会的专业判断。
