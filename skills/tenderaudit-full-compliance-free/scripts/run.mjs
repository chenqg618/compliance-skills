...
/**
 * run.mjs —— 招投标全案合规审计（免费）
 *
 * 全部检查都在**本机**完成：调用同目录下的 engine/tender-compliance-audit.js（纯 Node 标准库实现）。
 ...
 *   1  没给入参
 *   3  材料不足（空 / 只有空白 / 只有一个字符 / 没有任何可体检的内容）—— 此时不给结论
 *   4  入参文件读不到
 *   9  未预期错误
 */
...
const ENGINE = require(path.join(HERE, 'engine', 'tender-compliance-audit.js'));

const CAPABILITY = '招投标全案合规审计（免费）';
...
const USAGE = `招投标全案合规审计（免费） —— 本机执行的AI体检

  **完全免费**：不需要注册、不需要 API Key，材料不出本机（运行结束后有一次匿名次数统计，可用 --no-stats 关闭）。
...
function reportInsufficient(args, outcome, loaded) {
  const missing = (outcome && outcome.missing) || ['入参无法解析成可体检的材料'];
  const advice = (outcome && outcome.advice)
    || '请传 bidders 数组，每家一个 {name, items, text}；或先用 --sample 看看需要什么格式。';
  ...
  if (args.json) {
    console.log(JSON.stringify({
      ...
      missing,
      advice,
      note: '材料不足，本次没有执行任何检查，因此不出结论：既不做"有问题"的认定，也不做"干净"的认定。',
    }, null, 2));
    return 3;
  }
...
function main() {
  ...
  try {
    ...
    const mark = pathx.join(os.tmpdir(), `skillpay-choice-tenderaudit-full-compliance-free${sid ? '-' + sid : ''}.mark`);
  ...
  // __UPSELL__
  console.log('如需完整档：招投标全案合规审计 · 买断版（SkillPay 商品号 S0806000206880363）—— 本版本未执行的 4 项检查在完整档里都有。');
...
const HINT_ID = 'tenderaudit-full-compliance';
