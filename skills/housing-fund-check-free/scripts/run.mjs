...
/**
 * run.mjs —— 住房公积金汇缴与基数核对（免费）
 *
 * 全部检查都在**本机**完成：调用同目录下的 engine/housing-fund-check.js（纯 Node 标准库实现）。
...
const SAMPLE = { text: ENGINE.SAMPLE_TEXT };   // 样例本身就是一张带表头的住房公积金每月汇缴明细：缴存基数与上下限、单位与个人比例、补缴与断缴月份、新增/离职人员增减、单位合计与个人合计勾稽、与工资表应缴口径是否一致

const USAGE = `住房公积金汇缴与基数核对（免费） —— 本机执行的AI核对
...
入参写法：
  ...
  · 直接把住房公积金每月汇缴明细：缴存基数与上下限、单位与个人比例、补缴与断缴月份、新增/离职人员增减、单位合计与个人合计勾稽、与工资表应缴口径是否一致（**含表头**）从 Excel 复制成文本粘进来即可，Tab 分隔最稳

参数：
  -i, --input    入参 JSON 文件路径（也接受直接粘贴的住房公积金每月汇缴明细：缴存基数与上下限、单位与个人比例、补缴与断缴月份、新增/离职人员增减、单位合计与个人合计勾稽、与工资表应缴口径是否一致纯文本，需含表头）
      --sample   使用内置样例
...
function reportInsufficient(args, outcome, loaded) {
  ...
  const advice = (outcome && outcome.advice)
    || '请把住房公积金每月汇缴明细：缴存基数与上下限、单位与个人比例、补缴与断缴月份、新增/离职人员增减、单位合计与个人合计勾稽、与工资表应缴口径是否一致（含表头）贴进来：可用 {"text": "…"}，或先用 --sample 看看需要什么格式。';
  ...
  console.log('（检查项依赖你提供的住房公积金每月汇缴明细：缴存基数与上下限、单位与个人比例、补缴与断缴月份、新增/离职人员增减、单位合计与个人合计勾稽、与工资表应缴口径是否一致：材料不足或认不出表头时不做任何认定，也不套用默认值。）');
...
function main() {
  ...
  if (args.input) {
    ...
    console.error('缺少入参。用 --input 指定住房公积金每月汇缴明细：缴存基数与上下限、单位与个人比例、补缴与断缴月份、新增/离职人员增减、单位合计与个人合计勾稽、与工资表应缴口径是否一致文件，或用 --sample 自检。');
    console.error('');
    console.error(USAGE);
    return 1;
  }
  ...
  try {
    ...
    const mark = pathx.join(os.tmpdir(), `skillpay-choice-housing-fund-check-free${sid ? '-' + sid : ''}.mark`);
...
const HINT_ID = 'housing-fund-check';
