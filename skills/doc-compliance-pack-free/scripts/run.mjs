...
/**
 * run.mjs —— 发票与单据合规技能包（免费版）
 *
 * 全部检查都在**本机**完成：调用同目录下的 engine/doc-compliance-pack.js（纯 Node 标准库实现）。
 * 那个引擎内部装的是仓库里**已有的 14 个单据/材料合规检查引擎**（逐字节拷贝，规则一行没改），
 * 本包只做"材料目录 → 逐份跑 14 项单据合规检查 → 每份材料一行结论"这件编排工作。
 *
 * 刻意不做的事：
 ...
 *
 * 用法：
 *   node scripts/run.mjs --sample                  # 单份材料样例（内置，14 项检查全跑）
 *   node scripts/run.mjs --sample-batch            # 3 份材料的批量样例（干净 / 有硬伤 / 未交材料）
 *   node scripts/run.mjs --input 材料目录            # 批量：每个子目录 = 一份材料/一个供应商
 *   node scripts/run.mjs --input 单份材料.json       # 单份材料：JSON 或纯文本
...
const ENGINE = require(path.join(HERE, 'engine', 'doc-compliance-pack.js'));

const CAPABILITY = '发票与单据合规技能包（免费版）';

const NOTE = '本次按材料逐份跑全部 14 个单据合规检查项，全部在本机完成（核对过程不外发材料、不写文件）；'
  + '未执行的检查项已如实列出，不会用默认值编造结论。';

const TEXT_EXT = new Set(['.txt', '.tsv', '.csv', '.md', '.text', '.tab', '.log', '.dat', '.json']);

const SAMPLE = { text: ENGINE.SAMPLE_TEXT, object_name: '样例材料（单份模式）' };
const SAMPLE_BATCH = { objects: ENGINE.SAMPLE_OBJECTS };

const USAGE = `发票与单据合规技能包（免费版） —— 本机执行的AI单据核对

  **不需要付款**：不需要注册、不需要 API Key，核对过程在本机完成，不写文件；材料不出本机。
  检查项（每份材料都跑这几项）：${ENGINE.CHECKS_GIVEN.join('、')}
  本版本不包含：${ENGINE.CHECKS_WITHHELD.join('、')}

用法：
  node scripts/run.mjs --sample                   单份材料样例（内置）
  node scripts/run.mjs --sample-batch             3 份材料的批量样例
  node scripts/run.mjs --input 材料目录            批量：每个子目录 = 一份材料
  node scripts/run.mjs --input 材料.json           单份材料：JSON 或纯文本

入参写法：
  · 批量（推荐）：--input 指向一个目录，里面每个子目录就是一份材料，子目录里放这套单据
  · 单份材料：{"text":"（把材料按 === 检查项 === 分段贴进来，Tab 分隔最稳）"}

参数：
  -i, --input        入参 JSON 文件、纯文本文件，或**材料目录**
      --sample       使用内置单份材料样例
      --sample-batch 使用内置批量样例（3 份材料）
...
/** `--input <dir>`：每个子目录 = 一份材料；没有子目录时每个文件 = 一份材料。 */
function loadDir(dir) {
  const entries = listDir(dir);
  if (!entries) return { error: `读不到材料目录：${dir}` };
  ...
  return {
    payload: { objects },
    note: `目录 ${dir}：按${subdirs.length ? '子目录' : '文件'}识别到 ${objects.length} 份材料`,
  };
}
...
function reportInsufficient(args, outcome, loaded) {
  ...
  const advice = (outcome && outcome.advice)
    || '请用 --input 指向材料目录（每个子目录 = 一份材料），或先用 --sample / --sample-batch 看看需要什么格式。';
  ...
  console.log('（检查项依赖你提供的材料：一份材料都没有、或认不出任何一张单据时不做任何认定，也不套用默认值。）');
...
/** 每份材料一行的结论：材料名 / 各项通过或有问题 / 材料不足标「未执行」 */
...
function reportResult(view) {
  const r = view.result;
  console.log('免费AI单据核对完成（批量壳：一次跑完所有材料）');
  console.log(`本次执行的检查项（${view.checks_given.length} 个单据合规检查项）：${view.checks_given.join('、')}`);
  ...
  console.log('');
  console.log(`材料数 ${r.summary.objects}｜有问题 ${r.summary.objects_with_issues}｜干净 ${r.summary.objects_clean}｜未执行（没交材料）${r.summary.objects_not_run}`);
  console.log('');
  console.log('材料 × 结论（每份材料一行）');
  for (const o of r.objects) console.log(objectLine(o));
  if (r.findings.length) {
    console.log('');
    console.log(`问题明细（共 ${r.findings.length} 条，带材料 + 原文文件与行号）`);
...
function main() {
  ...
  if (args.input) {
    ...
    console.error('缺少入参。用 --input 指向材料目录或材料文件，或用 --sample / --sample-batch 自检。');
    console.error('');
    console.error(USAGE);
    return 1;
  }
  ...
  try {
    ...
    const mark = pathx.join(os.tmpdir(), `skillpay-choice-doc-compliance-pack-free${sid ? '-' + sid : ''}.mark`);
  ...
  // __UPSELL__
  console.log('如需完整档：发票与单据合规技能包 · 买断版（SkillPay 商品号 S0806000206877486）—— 本版本未执行的 4 项检查在完整档里都有。');
...
/* ⛔ 这里**不能**写 `process.exit(main())`（实测真缺陷）：买断包 `--json` 会把整本体检台账
   （3 份材料 ≈ 16 万字节）一起打出来，而 `process.exit()` 会**立刻**结束进程，
   stdout 是管道时**还没冲刷完的缓冲会被丢掉** —— 表现为输出被从中间截断，
   下一个人拿到的是 `Unterminated string in JSON`（而且**时好时坏**，是最难查的那种）。
   改成 `process.exitCode`：让事件循环自然排空，Node 会把 stdout 完整冲刷后再退出。 */
...
const HINT_ID = 'doc-compliance-pack';
