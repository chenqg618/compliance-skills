...
/**
 * run.mjs —— 制造与生产技能包（免费版）
 *
 * 全部检查都在**本机**完成：调用同目录下的 engine/manufacturing-ops-pack.js（纯 Node 标准库实现）。
 * 那个引擎内部装的是仓库里**已有的 14 个制造与生产检查引擎**（逐字节拷贝，规则一行没改），
...
const ENGINE = require(path.join(HERE, 'engine', 'manufacturing-ops-pack.js'));

const CAPABILITY = '制造与生产技能包（免费版）';

const NOTE = '本次按对象逐个跑全部 14 个制造与生产检查项，全部在本机完成（核对过程不外发材料、不写文件）；'
...
const USAGE = `制造与生产技能包（免费版） —— 本机执行的AI核对
...
入参写法：
  · 批量（推荐）：--input 指向一个目录，里面每个子目录就是一个对象，子目录里放该对象的一套制造与生产材料
...
function reportResult(view) {
  ...
  console.log(`本次执行的检查项（${view.checks_given.length} 个制造与生产检查项）：${view.checks_given.join('、')}`);
...
function main() {
  ...
  try {
    ...
    const mark = pathx.join(os.tmpdir(), `skillpay-choice-manufacturing-ops-pack-free${sid ? '-' + sid : ''}.mark`);
  ...
  // __UPSELL__
  console.log('如需完整档：制造与生产技能包 · 买断版（SkillPay 商品号 S0806000206880372）—— 本版本未执行的 4 项检查在完整档里都有。');
...
const HINT_ID = 'manufacturing-ops-pack';
