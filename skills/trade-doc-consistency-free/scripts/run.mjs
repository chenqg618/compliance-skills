...
const ENGINE = require(path.join(HERE, 'engine', 'trade-doc-consistency.js'));

const CAPABILITY = '外贸单证单单一致核对（免费）';
...
function reportInsufficient(args, outcome, loaded) {
  ...
  const advice = (outcome && outcome.advice)
    || '请提供至少两张单据（一个文本里用空行分隔，或用 documents 数组），或先用 --sample 看看需要什么格式。';
...
function main() {
  ...
  try {
    ...
    const mark = pathx.join(os.tmpdir(), `skillpay-choice-trade-doc-consistency-free${sid ? '-' + sid : ''}.mark`);
  ...
  // __UPSELL__
  console.log('如需完整档：外贸单证单单一致核对 · 买断版（SkillPay 商品号 S0806000206877473）—— 本版本未执行的 8 项检查在完整档里都有。');
...
const HINT_ID = 'trade-doc-consistency';
