...
/**
 * run.mjs —— 差旅与通行费技能包（免费版）
 *
 * 全部检查都在**本机**完成：调用同目录下的 engine/travel-toll-pack.js（纯 Node 标准库实现）。
 * 那个引擎内部装的是仓库里**已有的 2 个差旅与通行检查引擎**（逐字节拷贝，规则一行没改），
 * 本包只做"部门目录 → 逐部门跑这 2 项检查 → **每家部门一行结论**"这件编排工作。
 *
 * 刻意不做的事：
 ...
 *
 * 用法：
 *   node scripts/run.mjs --sample                   # 单部门样例（内置，2 项检查全跑）
 *   node scripts/run.mjs --sample-batch             # 3 家部门的批量样例（干净 / 有硬伤 / 未交材料）
 *   node scripts/run.mjs --input 部门目录            # 批量：每个子目录 = 一家部门/一套材料
 *   node scripts/run.mjs --input 单部门材料.json     # 单部门：JSON 或纯文本
 *   node scripts/run.mjs --input 部门目录 --json     # 机器可读
 *
 * 部门目录怎么放（每个子目录里放这家部门的一套材料，每项用 `=== 检查项 ===` 分段，Tab 分隔最稳）：
 *   部门目录/
 *     豫州销售一部（2026-04 报销）/差旅材料.txt
 *     中岳工程二部（2026-04 报销）/差旅材料.txt
 *     （目录里没有子目录时，直接把每个文件当一家部门）
 *   也可以把 14 张表拆成 2 个文件放进同一个子目录 —— 引擎会自己认表头找对应的那一项。
 *
 * 退出码：
 *   0  已执行检查（结果里有问题项或没有问题项都算执行成功）
 *   1  没给入参
 *   3  材料不足（一家部门的材料都没有 / 空 / 只有空白 / 认不出任何一张表）—— 此时不给任何结论
 *   4  入参文件或目录读不到 / 内容无法解析
 *   9  未预期错误
 */
...
const ENGINE = require(path.join(HERE, 'engine', 'travel-toll-pack.js'));

const CAPABILITY = '差旅与通行费技能包（免费版）';

const NOTE = '本次按部门逐个跑全部 2 个差旅与通行检查项，全部在本机完成（核对过程不外发材料、不写文件）；'
  + '未执行的检查项已如实列出，不会用默认值编造结论。';

const TEXT_EXT = new Set(['.txt', '.tsv', '.csv', '.md', '.text', '.tab', '.log', '.dat', '.json']);

const SAMPLE = { text: ENGINE.SAMPLE_TEXT, object_name: '样例部门（单部门模式）' };
const SAMPLE_BATCH = { objects: ENGINE.SAMPLE_OBJECTS };

const USAGE = `差旅与通行费技能包（免费版） —— 本机执行的AI核对

  **不需要付款**：不需要注册、不需要 API Key，核对过程在本机完成，不写文件；材料不出本机。
  检查项（每家部门都跑这几项）：${ENGINE.CHECKS_GIVEN.join('、')}
  本版本不包含：${ENGINE.CHECKS_WITHHELD.join('、')}

用法：
  node scripts/run.mjs --sample                   单部门样例（内置）
  node scripts/run.mjs --sample-batch             3 家部门的批量样例
  node scripts/run.mjs --input 部门目录            批量：每个子目录 = 一家部门
  node scripts/run.mjs --input 材料.json           单部门：JSON 或纯文本

入参写法：
  · 批量（推荐）：--input 指向一个目录，里面每个子目录就是一家部门，子目录里放这家部门的一套材料
  · 单部门：{"text":"（把这家部门的表按 === 检查项 === 分段贴进来，Tab 分隔最稳）"}

参数：
  -i, --input        入参 JSON 文件、纯文本文件，或**部门目录**
      --sample       使用内置单部门样例
      --sample-batch 使用内置批量样例（3 家部门）
...
/** `--input <dir>`：每个子目录 = 一家部门；没有子目录时每个文件 = 一家部门。 */
function loadDir(dir) {
  const entries = listDir(dir);
  if (!entries) return { error: `读不到部门目录：${dir}` };
  ...
  return {
    payload: { objects },
    note: `目录 ${dir}：按${subdirs.length ? '子目录' : '文件'}识别到 ${objects.length} 家部门`,
  };
}
...
function reportInsufficient(args, outcome, loaded) {
  const missing = (outcome && outcome.missing) || ['入参无法解析成可核对的部门材料'];
  const advice = (outcome && outcome.advice)
    || '请用 --input 指向部门目录（每个子目录 = 一家部门），或先用 --sample / --sample-batch 看看需要什么格式。';
  ...
  console.log('（检查项依赖你提供的部门材料：一家部门的材料都没有、或认不出任何一张表时不做任何认定，也不套用默认值。）');
...
/** 每家部门一行的结论：部门名 / 各项通过或有问题 / 材料不足标「未执行」 */
...
function reportResult(view) {
  const r = view.result;
  console.log('免费AI核对完成（批量壳：一次跑完所有部门）');
  console.log(`本次执行的检查项（${view.checks_given.length} 个差旅与通行检查项）：${view.checks_given.join('、')}`);
  ...
  console.log('');
  console.log(`部门数 ${r.summary.objects}｜有问题 ${r.summary.objects_with_issues}｜干净 ${r.summary.objects_clean}｜未执行（没交材料）${r.summary.objects_not_run}`);
  console.log('');
  console.log('部门 × 结论（每家部门一行）');
  for (const o of r.objects) console.log(objectLine(o));
  if (r.findings.length) {
    console.log('');
    console.log(`问题明细（共 ${r.findings.length} 条，带部门 + 原文文件与行号）`);
...
function main() {
  ...
  if (args.input) {
    ...
    console.error('缺少入参。用 --input 指向部门目录或材料文件，或用 --sample / --sample-batch 自检。');
    console.error('');
    console.error(USAGE);
    return 1;
  }
  ...
  try {
    ...
    const mark = pathx.join(os.tmpdir(), `skillpay-choice-travel-toll-pack-free${sid ? '-' + sid : ''}.mark`);
  ...
  // __UPSELL__
  console.log('如需完整档：差旅与通行费技能包 · 买断版（SkillPay 商品号 S0806000206877508）—— 本版本未执行的 4 项检查在完整档里都有。');
...
const HINT_ID = 'travel-toll-pack';
