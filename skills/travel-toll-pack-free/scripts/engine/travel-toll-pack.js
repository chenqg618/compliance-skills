 ...
 *
 * 这个包做的事**只有一件**（不是重写任何规则）：把仓库里**已有的 2 个差旅与通行检查引擎**
 * 装进一个批量壳里，`--input <目录>`（每个子目录 = 一家部门 / 一套材料）一次跑完所有部门，
 * **每家部门一行结论**。
 *   检查项：差旅费标准与超标核对、通行费与过路过桥核对。
 *   规则来源：`scripts/engine/parts/<检查项>/`（**逐字节拷贝**自那 2 个免费包，一行都没改；
...
/* 2 个成员检查项的**中文名 / base 名 / 引擎**（引擎逐字节来自各自免费包，本包一行规则都没改） */
const MEMBERS = [
  {
    label: '差旅费标准与超标核对',
  ...
  {
    label: '通行费与过路过桥核对',
    id: 'toll-processing-manual-check',
    entry: require('./parts/toll-processing-manual-check/toll-processing-manual-check.js'),
  },
];
...
/* 如实列出**每个成员检查包自己**没做的子检查（那 2 个免费包各自的未执行项）——
...
/* 有一处硬伤的样例：把「通行费与过路过桥核对」的合计行里 `46000` 改成 `46100.00`（各明细行相加 ≠ 合计）⇒ 必须报出来，并带**原文行号**（第 8 行）。 */
const DIRTY_FROM = '合计				46000	10400	17000	9000		9600	9450	300	250	17250	172500.00	';
const DIRTY_TO = '合计				46100.00	10400	17000	9000		9600	9450	300	250	17250	172500.00	';
const DIRTY_TEXT = SAMPLE_TEXT.replace(DIRTY_FROM, DIRTY_TO);

/* 只覆盖 3 项的样例：用来演示"材料只覆盖了一部分"时，批量壳**明确报一条**，
   而不是把"跑了 3 项"说成"2 项都核过了"。 */
...
function run(payload) {
  ...
  const result = {
    ...
    note: `本次对 ${rows.length} 家部门逐个跑了 ${CHECKS_GIVEN.length} 个差旅与通行检查项（每个成员包内部还有 `
