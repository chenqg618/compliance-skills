 ...
 * run.mjs —— 银行流水对账核对（免费）
 *
 * 全部检查都在**本机**完成：调用同目录下的 engine/bank-reconciliation.js（纯 Node 标准库实现）。
...
const ENGINE = require(path.join(HERE, 'engine', 'bank-reconciliation.js'));

const CAPABILITY = '银行流水对账核对（免费）';

const NOTE = '本版本只执行上面列出的检查项，核对全部在本机完成（不发送你的材料；结束后有一次匿名次数统计）；'
  + '未执行的检查项已如实列出，不会用默认值编造结论。';

const SAMPLE = { text: ENGINE.SAMPLE_TEXT };   // 样例里「银行」与「账面」两节用空行分隔

const USAGE = `银行流水对账核对（免费） —— 本机执行的AI核对
...
入参两种写法：
  · {"bank":[{"date":"2026-05-08","amount":-1240.00,"memo":"酒店消费"}],"book":[…同样格式…]}
  · 或直接给纯文本：先写一行「银行」、再写一行「账面」，各自下面按行写「日期 金额 摘要」

参数：
  -i, --input    入参 JSON 文件路径（也接受直接粘贴的纯文本；
                 「银行」与「账面」两节用单独一行分隔）
      --sample   使用内置样例
...
function reportInsufficient(args, outcome, loaded) {
  ...
  const advice = (outcome && outcome.advice)
    || '请提供两侧记录（银行流水 + 企业账面；一个文本里分「银行」「账面」两节，或用 bank / book 数组），或先用 --sample 看看需要什么格式。';
  ...
  console.log('（检查项依赖你提供的流水与账面记录：材料不足时不做任何认定，也不套用默认值。）');
...
function main() {
  ...
  if (args.input) {
    ...
    console.error('缺少入参。用 --input 指定流水/账面文件，或用 --sample 自检。');
    console.error('');
    console.error(USAGE);
    return 1;
  }
  ...
  try {
    ...
    const mark = pathx.join(os.tmpdir(), `skillpay-choice-bank-reconciliation-free${sid ? '-' + sid : ''}.mark`);
...
const HINT_ID = 'bank-reconciliation';
