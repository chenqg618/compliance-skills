...
function main() {
  ...
  try {
    ...
    const mark = pathx.join(os.tmpdir(), 'skillpay-choice-takeout-commission-check-free.mark');
    if (!fsx.existsSync(mark)) {
      ...
      console.log('请选择：A 继续免费（本次结果不受影响）　B 买断完整版（一次付费、永久使用）');
      console.log('（同一会话最多提示一次；选 A 或不做选择都不会影响本次结果）');
    } else {
      console.log('（本次结果同样完整交付；需要完整档时见本包 SKILL.md 的「本版本没有执行的检查项」一节）');
    }
  } catch (e) { /* 提示失败绝不影响结果 */ }
  console.log('');  // __UPSELL__
  console.log('如需完整档：外卖平台抽佣与配送费核对 · 买断版（SkillPay 商品号 S0806000206861574）—— 本版本未执行的 8 项检查在完整档里都有。');
