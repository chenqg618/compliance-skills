...
/**
 * run.mjs —— 环境保护税申报核对（免费）
 *
 * 全部检查都在**本机**完成：调用同目录下的 engine/environmental-tax-check.js（纯 Node 标准库实现）。
...
const ENGINE = require(path.join(HERE, 'engine', 'environmental-tax-check.js'));

const CAPABILITY = '环境保护税申报核对（免费）';

const NOTE = '本版本只执行上面列出的检查项，核对全部在本机完成（不发送你的材料；结束后有一次匿名次数统计）；'
  + '未执行的检查项已如实列出，不会用默认值编造结论。';

const SAMPLE = { text: ENGINE.SAMPLE_TEXT };   // 样例本身就是一张带表头的环保税申报表：应税污染物种类与排放量、污染当量数与当量值、适用税额、减免（低于标准/综合利用）、与监测报告或排污许可数据勾稽、按期申报与已缴金额

const USAGE = `环境保护税申报核对（免费） —— 本机执行的AI核对
...
入参写法：
  ...
  · 直接把环保税申报表：应税污染物种类与排放量、污染当量数与当量值、适用税额、减免（低于标准/综合利用）、与监测报告或排污许可数据勾稽、按期申报与已缴金额（**含表头**）从 Excel 复制成文本粘进来即可，Tab 分隔最稳

参数：
  -i, --input    入参 JSON 文件路径（也接受直接粘贴的环保税申报表：应税污染物种类与排放量、污染当量数与当量值、适用税额、减免（低于标准/综合利用）、与监测报告或排污许可数据勾稽、按期申报与已缴金额纯文本，需含表头）
      --sample   使用内置样例
...
function reportInsufficient(args, outcome, loaded) {
  ...
  const advice = (outcome && outcome.advice)
    || '请把环保税申报表：应税污染物种类与排放量、污染当量数与当量值、适用税额、减免（低于标准/综合利用）、与监测报告或排污许可数据勾稽、按期申报与已缴金额（含表头）贴进来：可用 {"text": "…"}，或先用 --sample 看看需要什么格式。';
  ...
  console.log('（检查项依赖你提供的环保税申报表：应税污染物种类与排放量、污染当量数与当量值、适用税额、减免（低于标准/综合利用）、与监测报告或排污许可数据勾稽、按期申报与已缴金额：材料不足或认不出表头时不做任何认定，也不套用默认值。）');
...
function main() {
  ...
  if (args.input) {
    ...
    console.error('缺少入参。用 --input 指定环保税申报表：应税污染物种类与排放量、污染当量数与当量值、适用税额、减免（低于标准/综合利用）、与监测报告或排污许可数据勾稽、按期申报与已缴金额文件，或用 --sample 自检。');
    console.error('');
    console.error(USAGE);
    return 1;
  }
  ...
  if (loaded && loaded.note) console.log(`（${loaded.note}）`);
  console.log('');
  // 免费档出口：只给商品名 + 官方购买入口（**不写任何商品号**，免得写死别家的号）
  console.log('');
console.log(JSON.stringify(view.result, null, 2));
  ...
  try {
    ...
    const mark = pathx.join(os.tmpdir(), `skillpay-choice-environmental-tax-check-free${sid ? '-' + sid : ''}.mark`);
...
const HINT_ID = 'environmental-tax-check';
