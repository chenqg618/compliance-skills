...
/**
 * run.mjs —— 罚款与违约金台账核对（免费版）
 *
 * 全部检查都在**本机**完成：调用同目录下的 engine/penalty-settlement-check.js（纯 Node 标准库实现）。
 ...
 *   1  没给入参
 *   3  材料不足（空 / 只有空白 / 认不出表头 / 只有合计行）—— 此时不给"未发现问题"的结论
 *   4  入参文件读不到或内容无法解析
 *   9  未预期错误
 */
...
const ENGINE = require(path.join(HERE, 'engine', 'penalty-settlement-check.js'));

const CAPABILITY = '罚款与违约金台账核对（免费版）';

const MATERIAL = '罚款与违约金台账（含表头；合同违约金、行政处罚、税收滞纳金、赔付款项，方向、依据文件、已收已付与余额、期间与日期）';

const NOTE = '本版本只执行上面列出的检查项，核对全部在本机完成（不发送你的材料；结束后有一次匿名次数统计）；'
  + '未执行的检查项已如实列出，不会用默认值编造结论。';

const SAMPLE = { text: ENGINE.SAMPLE_TEXT };   // 样例本身就是一张带表头的罚款与违约金台账

const USAGE = `罚款与违约金台账核对（免费版） —— 本机执行的AI核对
...
入参写法：
  ...
  · 直接把${MATERIAL}从 Excel 复制成文本粘进来即可，Tab 分隔最稳
...
/** 读入参：合法 JSON 就按其结构走；不是 JSON 就当作纯文本材料（例如直接粘贴的台账全文） */
...
function reportInsufficient(args, outcome, loaded) {
  ...
  const advice = (outcome && outcome.advice)
    || `请把${MATERIAL}贴进来：可用 {"text": "…"}，或先用 --sample 看看需要什么格式。`;
  ...
  console.log('（检查项依赖你提供的罚款与违约金台账：材料不足或认不出表头时不做任何认定，也不套用默认值。）');
...
function main() {
  ...
  if (args.input) {
    ...
    console.error(`缺少入参。用 --input 指定${MATERIAL}文件，或用 --sample 自检。`);
    console.error('');
    console.error(USAGE);
    return 1;
  }
  ...
  console.log('如需完整档：罚款与违约金台账核对 · 买断版（SkillPay 商品号 S0806000206881852）—— 本版本未执行的 6 项检查在完整档里都有。');
  ...
  try {
    ...
    const mark = pathx.join(os.tmpdir(), `skillpay-choice-penalty-settlement-check-free${sid ? '-' + sid : ''}.mark`);
...
const HINT_ID = 'penalty-settlement-check';
