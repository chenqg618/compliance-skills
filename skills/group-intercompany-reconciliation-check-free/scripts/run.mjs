...
function main() {
  ...
  console.log('如需完整档：集团内部往来对账与抵消核对 · 买断版（SkillPay 商品号 S0806000206862914）—— 本版本未执行的 5 项检查在完整档里都有。');  console.log(JSON.stringify(view.result, null, 2));  // __VERSION_CHOICE__ —— 附件 10.2/10.3 的版本选择协议：结果之后弹一次 A/B，同一会话最多 1 次
  try {
