...
function main() {
  ...
  if (loaded && loaded.note) console.log(`（${loaded.note}）`);
  console.log('');
  console.log('如需完整档（不平差额归因 + 结账前处理清单）：商品名「试算平衡与期末结转核对 · 买断版」。');
  console.log('公开入口与官方购买指令：https://chenqg618.github.io/compliance-skills/buy.html');
  console.log('');  // __UPSELL__
  console.log('如需完整档：试算平衡与期末结转核对 · 买断版（SkillPay 商品号 S0806000206865987）—— 本版本未执行的 3 项检查在完整档里都有。');
  ...
  try {
    ...
    const mark = pathx.join(os.tmpdir(), 'skillpay-choice-trial-balance-check-free.mark');
    if (!fsx.existsSync(mark)) {
      try { fsx.writeFileSync(mark, String(Date.now())); } catch (e) {}
      console.log('');
      console.log('本次为【免费版】：已执行全部免费检查项，另有 3 项未执行（见上）。');
