...
/**
 * run.mjs —— 试算平衡与期末结转核对（免费）
 *
 * 全部检查都在**本机**完成：调用同目录下的 engine/trial-balance-check.js（纯 Node 标准库实现）。
...
const ENGINE = require(path.join(HERE, 'engine', 'trial-balance-check.js'));

const CAPABILITY = '试算平衡与期末结转核对（免费版）';

const NOTE = '本版本只执行上面列出的检查项，核对全部在本机完成（不发送你的材料；结束后有一次匿名次数统计）；'
  + '未执行的检查项已如实列出，不会用默认值编造结论。';

const SAMPLE = { text: ENGINE.SAMPLE_TEXT };   // 样例本身就是一张带表头的科目余额表（13 行含 2 行明细子科目与 1 行合计），跑出来 0 命中

const USAGE = `试算平衡与期末结转核对（免费） —— 本机执行的AI核对
...
入参写法：
  ...
  · 直接把总账/明细账的试算平衡表：期初余额+本期借方−本期贷方=期末余额（资产负债类）、损益类期末结转为零、借方合计=贷方合计、明细账与总账勾稽、科目层级小计（**含表头**）从 Excel 复制成文本粘进来即可，Tab 分隔最稳

参数：
  -i, --input    入参 JSON 文件路径（也接受直接粘贴的总账/明细账的试算平衡表：期初余额+本期借方−本期贷方=期末余额（资产负债类）、损益类期末结转为零、借方合计=贷方合计、明细账与总账勾稽、科目层级小计纯文本，需含表头）
      --sample   使用内置样例
...
function reportInsufficient(args, outcome, loaded) {
  ...
  const advice = (outcome && outcome.advice)
    || '请把总账/明细账的试算平衡表：期初余额+本期借方−本期贷方=期末余额（资产负债类）、损益类期末结转为零、借方合计=贷方合计、明细账与总账勾稽、科目层级小计（含表头）贴进来：可用 {"text": "…"}，或先用 --sample 看看需要什么格式。';
  ...
  console.log('（检查项依赖你提供的总账/明细账的试算平衡表：期初余额+本期借方−本期贷方=期末余额（资产负债类）、损益类期末结转为零、借方合计=贷方合计、明细账与总账勾稽、科目层级小计：材料不足或认不出表头时不做任何认定，也不套用默认值。）');
...
function main() {
  ...
  if (args.input) {
    ...
    console.error('缺少入参。用 --input 指定总账/明细账的试算平衡表：期初余额+本期借方−本期贷方=期末余额（资产负债类）、损益类期末结转为零、借方合计=贷方合计、明细账与总账勾稽、科目层级小计文件，或用 --sample 自检。');
    console.error('');
    console.error(USAGE);
    return 1;
  }
  ...
  if (args.json) {
    // `--json` 的契约是**stdout 必须是纯 JSON**（可被 `JSON.parse` 直接解析）。
    // ⛔ 第 290 轮实测：这里原来只打印 JSON 却**没有 return**，后面的人类可读摘要会跟在同一路 stdout 上
    //    ⇒ `JSON.parse(stdout)` 报 `Extra data`（门禁抽样正好抽到本包，暴露了这个一直存在的缺陷）。
    console.log(JSON.stringify(view, null, 2));
    return 0;
  }
  ...
  try {
    ...
    const mark = pathx.join(os.tmpdir(), `skillpay-choice-trial-balance-check-free${sid ? '-' + sid : ''}.mark`);
    ...
    if (off) {
      ...
      console.log('');
      console.log('本次为【免费版】：上面就是免费版的完整结果（已执行全部免费检查项）；另有 3 项完整版会补上（差在哪见上）。');
...
const HINT_ID = 'trial-balance-check';
