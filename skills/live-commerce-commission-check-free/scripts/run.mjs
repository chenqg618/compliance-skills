...
function main() {
  ...
  if (loaded && loaded.note) console.log(`（${loaded.note}）`);
  console.log('');
  console.log(`如需完整档：本版本未执行的 ${view.checks_withheld.length} 项检查（${view.checks_withheld.join('、')}）在买断版里都会执行。`);
  console.log('公开入口：https://chenqg618.github.io/compliance-skills/buy.html（买断商品上架后，本包 SKILL.md 会给出官方购买指令与商品号）。');
  console.log('');  // __UPSELL__
  console.log('如需完整档：直播佣金与坑位费结算核对 · 买断版（SkillPay 商品号 S0806000206845551）—— 本版本未执行的 5 项检查在完整档里都有。');
  ...
  try {
    ...
    const mark = pathx.join(os.tmpdir(), 'skillpay-choice-live-commerce-commission-check-free.mark');
